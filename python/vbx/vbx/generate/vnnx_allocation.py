...
def fia_input_shaper_size_kb(filter_copies, sparse):
    return (
        filter_copies
        * get_input_shaper_banks(sparse)
        * get_input_shaper_banks_kb(sparse)
    )
...
def fia_weight_shaper_size_kb(parallel_kernels, sparse):
    return (
        parallel_kernels * get_weight_shaper_banks(sparse) * WEIGHT_SHAPER_BANK_SIZE_KB
    )
...
def _get_input_and_weight_shaper_bank_sizes(
    filter_copies, parallel_output_maps, sparse
):
...
def sp_used_fn(node):
    if node.Conv2DOptions.mxp_double_buffer:
        return lambda o, t0, t: (
            o * 2 + t0
            if (len(node.subnode_array) == 0 and node.output_strides == [1, 1])
            else max(o * 3 + t, o * 2 + t0 + t)
        )
    else:
        return lambda o, t0, t: (
            o + t0
            if (len(node.subnode_array) == 0 and node.output_strides == [1, 1])
            else max(o * 2 + t, o + t0 + t)
        )
...
def get_output_shapes(conv8, input_height, input_width):
    ...
    dilation_height_factor, dilation_width_factor = (
        conv8.dilation_height_factor,
        conv8.dilation_width_factor,
    )
...
def is_sp_invalid(node, preset, sp, tile, sparse=None):
    v_out, v_tmp0, v_tmp = max_scratchpad_required(node, preset, tile, sparse=sparse)
...
def increment_tile(
    node, preset, sp, fx, tile, idx, limit, scale=1, offset=1, sparse=None
):
    tile = tile.copy()

    if tile[idx] < limit:
        tmp = tile[idx]
        sp_invalid = is_sp_invalid(node, preset, sp, tile, sparse=sparse)
    ...
    while (not np.all(fx(tile) >= 1)) and (tile[idx] > 1):
        tile[idx] -= 1
...
def minimum_tile_subgraph(node, fx):
    MAPS, ROWS, COLUMNS = 4, 5, 6

    tile = np.array([1, 1, 1, 1, 1, 1, 1])
    j = 0
    while not np.all(fx(tile) >= 1):
        for i, t in enumerate(fx(tile)):
            if i == COLUMNS and t > node.n:
                tile[i] = 0
                return tile
            if i == ROWS and t > node.m:
                tile[i] = 0
                return tile
            if i == MAPS and t > node.channels:
                tile[i] = 0
                return tile
            if t < 1:
                tile[i] += 1
...
def compute_sp_used(node, vl, tile, sparse=None):
    """Return scalar sp_used for given tile using existing helpers."""
    v_out, v_tmp0, v_tmp = max_scratchpad_required(node, vl, tile, sparse=sparse)
    return sp_used_fn(node)(v_out, v_tmp0, v_tmp)


def plan_sequantial_loads_from_min_tile(tile, node, vl, sp, sparse=None):
    """
    tile size is proportional to min_tile size, so we can estimate the number of trips needed along each dimension
    by dividing tile dimensions by min_tile dimensions
    ...
    while not sp_invalid:
        new_tile[-2] = tile[-2] // 2
        new_tile[-1] = tile[-1] // 2
        new_tile[-3] = tile[-3] // 2 if tile[-3] > 1 else 1
        rows_trips = tile[-2] // new_tile[-2]
        cols_trips = tile[-1] // new_tile[-1]
        maps_trips = tile[-3] // new_tile[-3] if tile[-3] > 1 else 1

        sp_invalid = is_sp_invalid(node, vl, sp, tile, sparse=sparse)

    return new_tile[-2], new_tile[-1], new_tile[-3], rows_trips, cols_trips, maps_trips


def minimum_valid_tile_subgraph(
    node,
    preset,
    sp,
    fx,
...
def max_scratchpad_required(node, preset, tile, sp=None, sparse=None):
    tile = tile.copy()
    v_out, v_tmp0 = max_sp_tile(node, preset, sp)(tile)
    tile = max_output_tile(node, sparse=sparse)(tile)

    v_tmp = 0
    for sn in node.subnode_array:
        v_0, v_1 = max_sp_tile(sn, preset, sp)(tile)
...
def min_output_tile(node):
    ...
    if e in default_types:
        ...
        dhf, dwf = (
            node.Conv2DOptions.dilation_height_factor,
            node.Conv2DOptions.dilation_width_factor,
        )
        ...
        if e in [
            ...
            assert node.reduce8.channels

        def x(tile):
            batch, imaps, irows, icols, maps, rows, cols = tile
            _, _, _, _, omaps, orows, ocols = tile
            if node.reduce8.columns:
                ocols = 1
            if node.reduce8.rows:
                orows = 1
            if node.reduce8.channels:
                omaps = 1

            if node.reduce8.columns and cols != node.tensor_array[0].shape[-1]:
                return np.asarray([batch, imaps, irows, icols, omaps, orows, 0])
            if node.reduce8.rows and rows != node.tensor_array[0].shape[-2]:
                return np.asarray([batch, imaps, irows, icols, omaps, 0, ocols])
            if node.reduce8.channels and maps != node.tensor_array[0].shape[-3]:
                return np.asarray([batch, imaps, irows, icols, 0, orows, ocols])
        ...
        def x(tile):
            ...
            if cols != node.tensor_array[0].shape[-1]:
                return np.asarray(
                    [batch, imaps, irows, icols, maps, (rows - kh) // sh + 1, 0]
                )

            if OPTIMIZED_AVERAGE_POOL:
                if (
                    node.tensor_array[0].shape[-3] % 2 == 0
                    and kh == 2
                    and kw == 2
                    and sh == 1
                    and sw == 1
                ...
                if (
                    node.tensor_array[0].shape[-2] % 2 == 0
                    and node.tensor_array[0].shape[-1] % 2 == 0
                    and kh == 2
                    and kw == 2
                    and sh == 2
                    and sw == 2
                    ...
                    if rows % 2 != 0:
                        return np.asarray(
                            [batch, imaps, irows, icols, maps, 0, (cols - kw) // sw + 1]
                        )
            return np.asarray(
                [
                    batch,
        ...
        return lambda s: np.concatenate(
            (s[:ROWS], [(s[ROWS] - kh) // sh + 1], [(s[COLUMNS] - kw) // sw + 1])
        )
        ...
        def x(tile):
            ...
            return np.asarray(
                [batch, imaps, irows, icols, maps // (r**2), rows * r, cols * r]
            )
        ...
        if e in [BuiltinOperator.PAD, BuiltinOperator.DILATE]:
            dh, dw = (
                node.PadOptions.transpose_dilate_h,
                node.PadOptions.transpose_dilate_w,
            )
            dhf = dh - 1
            dwf = dw - 1
        ...
        def x(tile):
            ...
            if axis == -4:
                return np.asarray(
                    [count * batch, imaps, irows, icols, maps, rows, cols]
                )
            elif axis == -3:
                return np.asarray(
                    [batch, imaps, irows, icols, count * maps, rows, cols]
                )
            elif axis == -2:
                return np.asarray(
                    [batch, imaps, irows, icols, maps, count * rows, cols]
                )
            elif axis == -1:
                return np.asarray(
                    [batch, imaps, irows, icols, maps, rows, count * cols]
                )
        ...
        def x(tile):
            ...
            if axis == -3:
                if maps < node.tensor_array[0].shape[-3]:
                    return np.asarray(
                        [batch, imaps, irows, icols, 0, rows, cols]
                    )  # need full maps
            if axis == -1:
                if cols < node.tensor_array[0].shape[-1]:
                    return np.asarray(
                        [batch, imaps, irows, icols, maps, rows, 0]
                    )  # need full cols

            if axis == -2:
                if rows < node.tensor_array[0].shape[-2]:
                    return np.asarray(
                        [batch, imaps, irows, icols, maps, 0, cols]
                    )  # full rows
        ...
        def x(tile):
            ...
            temp_col = (cols) * sw
            in_rows, in_cols = (
                node.tensor_array[0].shape[-2],
                node.tensor_array[0].shape[-1],
            )
            ...
            if e == BuiltinOperator.RESIZE_BILINEAR:
                if rows < node.tensor_array[0].shape[-2] and rows > 0:
                    temp_row = (rows - 1) * sh
                else:
                    temp_row = rows * sh

                if cols < node.tensor_array[0].shape[-1] and cols > 0:
                    temp_col = (cols - 1) * sw
                else:
                    temp_col = cols * sw
                    # temp_col = (cols-1)*sw

                if (sh == 2.0 and sw == 2.0) or (sh == 4.0 and sw == 4.0) or (sh == 8.0 and sw == 8.0):
                    temp_row = (rows) * sh
                    temp_col = (cols) * sw

                    if temp_col != int(temp_col) or cols < 2:
                        return np.asarray(
                            [batch, imaps, irows, icols, maps, temp_row, 0]
                        )

                    if temp_row != int(temp_row) or rows < 2:
                        return np.asarray(
                            [batch, imaps, irows, icols, maps, 0, temp_col]
                        )
            ...
            return np.asarray(
                [batch, imaps, irows, icols, maps, int(temp_row), int(temp_col)]
            )
        ...
        def x(tile):
            ...
            filter_channels, filter_rows, filter_columns = (
                filter_dim[1],
                filter_dim[2],
                filter_dim[3],
            )
            ...
            omaps = maps
            if node.tensor_array[0].shape[-3] == maps and filter_channels > maps:
                omaps = filter_channels

            if e == BuiltinOperator.MUL:
                if (
                    filter_rows == node.tensor_array[0].shape[-2]
                    and filter_columns == node.tensor_array[0].shape[-1]
                ):
                    if cols != node.tensor_array[0].shape[-1]:
                        return np.asarray([batch, imaps, irows, icols, omaps, rows, 0])

            return np.asarray([batch, imaps, irows, icols, omaps, rows, cols])
...
def is_tile_valid_for_subgraph(node, tile, sparse=None):
    """Check if tile satisfies min_output_tile constraints for node and all subnodes.

    The tile transforms as it flows through the subgraph:
    - Input tile -> node -> transformed tile -> subnode1 -> transformed -> subnode2 -> ...
    Each stage must satisfy that subnode's min_output_tile constraint.
    """
    tile = tile.copy()

    # Check main node's constraint
    if not np.all(min_output_tile(node)(tile) >= 1):
        return False

    # Transform tile through main node
    tile = max_output_tile(node, sparse=sparse)(tile)

    # Check each subnode's constraint with the transformed tile
    for sn in node.subnode_array:
        if not np.all(min_output_tile(sn)(tile) >= 1):
            return False
        # Transform tile for next subnode
        tile = max_output_tile(sn, sparse=sparse)(tile)
...
def max_sp_tile(node, preset, sp=None):
    vector_lanes = preset_select["VECTOR_LANES"][preset]
    parallel_kernels = preset_select["PARALLEL_OUTPUT_MAPS"][preset]
    ...
    if e in default_types:
        ...
        if not node.Conv2DOptions.use_fia:  # vector
            ...
            def x(tile):
                ...
                v_tmp = 0
                if isinstance(type(node), Subnode):
            ...
            def x(tile):
                ...
                if not node.offloaded:
                    _, _, orows, ocols = get_output_shapes(
                        node.Conv2DOptions, rows, cols
                    )

                v_out = aligned(batch * maps * orows * ocols)
                if node.Conv2DOptions.use_depthwise:
                    # depthwise needs extra space to account for writing past "shaper" sizes
                    # parallel_kernels * ocols determined from depthwise quantizer stride D2 (assumes accums=1)
                    v_out = aligned(
                        (batch * maps * orows * ocols) + (parallel_kernels * ocols)
                    )
        ...
        if not node.Conv2DOptions.use_fia:
            ...
            def x(tile):
                ...
                v_out = aligned(
                    batch * maps * (transformed_rows * transformed_cols + out_offset)
                )
        ...
        def x(tile):
            ...
            v_out = aligned(omaps)
            v_tmp = aligned(omaps * 4)
            if isinstance(
                type(node), Subnode
            ):  # currently bring in single full map at a time
        ...
        def x(tile):
            batch, imaps, irows, icols, maps, rows, cols = tile
            v_tmp = (
                aligned(batch * maps * rows * cols * 4) * 2
            )  # 2 word size vectors (of input tile)
            v_out = aligned(
                batch * maps * rows * cols
            )  # byte size vector( same as input tile size)
        ...
        def x(tile):
            ...
            if node.prelu.optimized:
                ...
                if node.prelu.vci_int8 == -1:
                    v_tmp = (
                        aligned(
                            batch * min(maps, node.prelu.maps_at_once) * rows * cols * 2
                        )
                        * 2
                    )  # 2 half size vectors (of input tile)
                else:
                    v_tmp = (
                        aligned(
                            batch * min(maps, node.prelu.maps_at_once) * rows * cols * 2
                        ...
                        * 1
                    )  # 1 half size vectors (of input tile)
            else:
                v_tmp = (
                    aligned(
                        batch * min(maps, node.prelu.maps_at_once) * rows * cols * 4
                    )
                    * 2
                )  # 2 word size vectors (of input tile)
            v_out = aligned(
        ...
        def x(tile):
            batch, imaps, irows, icols, maps, rows, cols = tile
            v_tmp = aligned(
                batch * maps * rows * cols * 4 * 2
            )  # 2 word size vectors (of input tile)
            v_tmp = aligned(
                batch * maps * rows * cols * 4 * 8
            )  # vbx_MultiplyByQuantizedMultiplier_shift_less_than_1 tmp
            v_tmp = aligned(batch * maps * rows * cols * 2)
            v_out = aligned(
                batch * maps * rows * cols
            )  # byte size vector( same as input tile size)
        ...
        BuiltinOperator.SPLIT,
        BuiltinOperator.SPLIT_V,
    ]:  # currently vnnx_subgraph, allocating v_in
        ...
        def x(tile):
            ...
            v_out = aligned(batch * omaps * rows * cols)
        ...
        def x(tile):
            batch, imaps, irows, icols, maps, rows, cols = tile
            v_tmp = aligned(
                tile_n * batch * tile_c * maps * tile_h * rows * tile_w * cols
            )
            v_out = aligned(
                tile_n * batch * tile_c * maps * tile_h * rows * tile_w * cols
            )
        ...
        def x(tile):
            batch, imaps, irows, icols, maps, rows, cols = tile
            if isinstance(node, Node) and node.ResizeOptions.min_cols is not None:
                cols = node.ResizeOptions.min_cols
                rows = node.ResizeOptions.min_rows
                maps = node.ResizeOptions.min_maps
            size = batch * maps * rows * cols
            v_tmp_size = max(rows, int(cols * sw))
            v_tmp = aligned(v_tmp_size * 4)
            v_tmp += aligned(cols * sw * rows * sh)

            if sh==8.0 or sh == 4.0 or sh == 2.0:
            ...
            # Add input buffer for in_scratchpad mode full map
        ...
        def x(tile):
            ...
            # level 2, 3, 4 optimization for add/sub
            elif node.eltwise8.optimized in [2, 3, 4]:
                node.eltwise8.isize = 0
                v_tmp += aligned(batch * maps * rows * cols * 2)  # v_add

            elif node.eltwise8.type in [
                eltwise_type.ELTWISE_MAXIMUM,
                eltwise_type.ELTWISE_MINIMUM,
            ]:
                v_tmp = aligned(batch * maps * rows * cols)  # v_in
                node.eltwise8.isize = 0
            else:
...
def valid_conv_rows(tile, conv_rows, node, preset, sparse=None):
    ...
    batch, imaps, irows, icols, maps, rows, cols = tile
    input_shaper_size, _ = _get_input_and_weight_shaper_sizes(
        filter_copies, parallel_output_maps, sparse
    )
...
def pprint_tile(node, tile):
    BATCH, IMAPS, IROWS, ICOLUMNS, MAPS, ROWS, COLUMNS = 0, 1, 2, 3, 4, 5, 6

    node_name = str(node.type).split(".")[-1]
    tile_nchw = (tile[MAPS], tile[IMAPS], tile[ROWS], tile[COLUMNS])

    graph_walk = (node.maps, node.rows, node.cols)
    graph_walk0 = (node.maps, node.rows_0, node.cols_0)
    graph_walkf = (node.maps, node.rows, node.cols)
    ...
    if node.type in [
        ...
        print(
            ...
            tile_nchw,
            "~#:",
            node.Conv2DOptions.tile_count,
            "m,r,c:",
            graph_walk0,
            "sp:",
            node.scratchpad_bytes,
            "sn:",
            len(node.subnode_array),
            "str:",
            node.output_strides,
        )
    else:
...
def set_tile_attr(node, preset, sp, tile, cores=1, sparse=None):
    v_out, v_tmp0, v_tmp = max_scratchpad_required(node, preset, tile, sparse=sparse)
        ...
        try:
            ...
            for yi, ystart in enumerate(y):
                ...
                for xi, xstart in enumerate(x):
                    ...
                    out_col = out[9]
                    print(
                        f"in y=[{ystart},{ystart+ylen-1}] x=[{xstart},{xstart+xlen-1}] -> out y=[{out_y},{out_y+out_r-1}] x=[{out_x},{out_x+out_col-1}]"
                    )
        except Exception:
            pass
    ...
    if e in [
        ...
        tile_count = (
            len(y)
            * len(x)
            * ((m_stop - m_start + maps - 1) // maps)
            * ((node.channels + imaps - 1) // imaps)
        )
        node.Conv2DOptions.tile_count = tile_count
    else:
        tile_count = len(y) * len(x) * ((m_stop - m_start + maps - 1) // maps)
...
def adjust_tile(node):
    ...
    if e in default_types:
        ...
        def adjust(layer):
            ...
            is_last_row_tile = y + r >= m
            is_last_col_tile = x + col >= n
            out_m = int(m * sh)
            out_n = int(n * sw)
            ...
            if e == BuiltinOperator.RESIZE_NEAREST_NEIGHBOR:
                ...
                if is_last_row_tile:
                    r = out_m - y  # Fill to output boundary
                else:
                    r = floor(r * sh)

                if is_last_col_tile:
                    col = out_n - x  # Fill to output boundary
                else:
                    col = floor(col * sw)
            ...
            # for each min input tile, we return th output tile size
                ...
                if (sw == 2.0 and sh == 2.0) or (
                    sh == 4.0 and sw == 4.0
                ):  # 2X or 4X testing
                    # Output position from input position
                    out_y = floor(y * sh)
                    out_x = floor(x * sw)


                # Output size depends on whether this tile reaches the input edge
                # For non-edge tiles, use (r-1)*scale because the last input row
                # needs its neighbor from the next tile for bilinear interpolation
                    if is_last_row_tile:
                        out_r = out_m - out_y  # Fill to output boundary
                    else:
                        out_r = floor(
                            (r - 1) * sh
                        )  # Leave room for next tile's overlap

                    if is_last_col_tile:
                        out_col = out_n - out_x
                    else:
                        out_col = floor((col - 1) * sw)

                else:  # non-integer scale factors
                    if y == 0:
                        out_y = floor(y * sh)
                    else:
                        out_y = floor((y) * sh)
...
def valid_fia_fc(accum_depth, rows, cols, preset, sparse=None, no_sublayers=True):
    ...
    input_shaper_size, weight_shaper_size = _get_input_and_weight_shaper_bank_sizes(
        filter_copies, parallel_output_maps, sparse
    )
    input_shaper_buffer_size = iw * rows
    weight_shaper_buffer_size = accum_depth * cols
    # account for padded weights for alignment
    weight_shaper_buffer_size += (
        (parallel_output_maps - (weight_shaper_buffer_size % parallel_output_maps))
        if (weight_shaper_buffer_size % parallel_output_maps)
        else 0
    )
    ...
    if weight_shaper_buffer_size > weight_shaper_size:
        return False
    if cols * QUANTIZATION_RECORD_WIDTH_BYTES > (
        fia_quantization_shaper_size_kb() * 1024
    ):  # cols*sizeof(quantization_record)
        return False
...
def valid_fia(
    ...
    # Get the input and weight shaper sizes
    input_shaper_size, weight_shaper_size = _get_input_and_weight_shaper_sizes(
        filter_copies, parallel_output_maps, sparse
    )
    ...
    if node.Conv2DOptions.use_depthwise:
        ...
        weight_shaper_buffer_size = (
            imaps * kw * ((kh + parallel_output_maps - 1) * parallel_output_maps)
        )
        if not use_db:
            if imaps * pcols * prows > input_shaper_size:  # input_shaper_size
                return False
            if split_weights:
                if weight_shaper_buffer_size > (
                    weight_shaper_size / 2
                ):  # weight_shaper_size
                    return False
            else:
            ...
            if weight_shaper_buffer_size > (
                ...
                return False
        if maps * QUANTIZATION_RECORD_WIDTH_BYTES > (
            fia_quantization_shaper_size_kb() * 1024
        ):  # quantization_shaper_size maps*sizeof(quantization_record))
            return False
    else:
        if node.Conv2DOptions.use_strided:
            srows = rows // sh
        ...
        input_imaps = imaps + (
            input_shaper_banks
            - (
                (imaps % input_shaper_banks)
                if (imaps % input_shaper_banks)
                else input_shaper_banks
            )
        )
        if not db_rows:
        ...
        weight_maps += parallel_output_maps - (
            (maps % parallel_output_maps)
            if (maps % parallel_output_maps)
            else parallel_output_maps
        )
        if use_db:
            if weight_maps * kh * kw * weight_imaps > (
                weight_shaper_size / 2
            ):  # weight_shaper_size
                return False
        elif split_weights:
            ...
            if (
                weight_maps * kh * kw * weight_imaps > weight_shaper_size
            ):  # weight_shaper_size
                return False
        if maps * QUANTIZATION_RECORD_WIDTH_BYTES > (
            fia_quantization_shaper_size_kb() * 1024
    ...
    if not node.Conv2DOptions.use_depthwise:
        input_shaper_bank_size, weight_shaper_bank_size = (
            _get_input_and_weight_shaper_bank_sizes(
                filter_copies, parallel_output_maps, sparse
            )
        )

        if input_imaps % input_shaper_banks != 0:
            return False
        ...
        if not db_rows:
            if not node.Conv2DOptions.use_strided:
                ...
                if use_db:
                    if elements * sh * sw * imaps_per_in_bank > (
                        input_shaper_bank_size / 2
                    ):
                        return False
                else:
        ...
        if weight_imaps % WEIGHT_SHAPER_DATA_BANKS != 0:
            return False
        imaps_per_weight_bank = weight_imaps // WEIGHT_SHAPER_DATA_BANKS
        if use_db:
            if weight_maps * kh * kw * imaps_per_weight_bank > (
                weight_shaper_bank_size / 2
            ):
                return False
        elif split_weights:
            if weight_maps * kh * kw * imaps_per_weight_bank > (
...
def fit_depthwise(tile, node, sp, fx, preset, use_db=None, row_db=None, sparse=None):
    v = lambda m, im, r, c: valid_fia(
        m, im, r, c, node, preset, use_db, row_db, sparse=sparse
    )
    BATCH, IMAPS, IROWS, ICOLUMNS, MAPS, ROWS, COLUMNS = 0, 1, 2, 3, 4, 5, 6
    t = tile.copy()
    ...
    while limit_cols > t[COLUMNS] and not v(t[MAPS], t[IMAPS], t[ROWS], limit_cols):
        limit_cols -= 1
    t = increment_tile(
        node, preset, sp, fx, t, COLUMNS, limit=limit_cols, sparse=sparse
    )
    ...
    while limit_rows > t[ROWS] and not v(t[MAPS], t[IMAPS], limit_rows, t[COLUMNS]):
        limit_rows -= 1
    t = increment_tile(node, preset, sp, fx, t, ROWS, limit=limit_rows, sparse=sparse)
    ...
    while True:
        ...
        if not v(next_maps, t[IMAPS], t[ROWS], t[COLUMNS]):
            break
        next_tile = increment_tile(
            node, preset, sp, fx, t, MAPS, limit=next_maps, sparse=sparse
        )
    ...
    while True:
        ...
        if not v(t[MAPS], next_imaps, t[ROWS], t[COLUMNS]):
            break
        next_tile = increment_tile(
            node, preset, sp, fx, t, IMAPS, limit=next_imaps, sparse=sparse
        )
...
def fit_conv(
    ...
    def is_tile_le(a, b):
        """Return True if tile a < tile b in every dimension."""
        a = np.asarray(a)
        b = np.asarray(b)
        return np.any(a < b)
    ...
    # standard case: init maps=parallel_output_maps,imaps=padded_channels, then max columns, imaps, rows, map sets
    t, t_count, is_valid = conv_tile(
        tile, node, sp, fx, preset, use_db, row_db, split_weights, sparse
    )

    # 2 alternative cases where rows maximized before imaps (but only used if less tiles required)
    if k > 1 and k < parallel_output_maps:  # small output channels
        alt, alt_count, is_valid = conv_tile(
            tile,
            node,
            sp,
            fx,
            ...
            split_weights,
            sparse,
            full_rows_first=True,
        )
        if alt_count < t_count:
            t = alt
    elif kh > 1 and (k * c > 256**2 or dh > 1):  # large weights or dilated
        alt, alt_count, is_valid = conv_tile(
            tile,
            node,
            sp,
            ...
            split_weights,
            sparse,
            partial_rows_first=True,
        )
        if alt_count < t_count:
            t = alt

    BATCH, IMAPS, IROWS, ICOLUMNS, MAPS, ROWS, COLUMNS = 0, 1, 2, 3, 4, 5, 6
    if is_tile_le(t, tile) or not is_valid:
        # if the return tile is smaller than the minimum input tile, then return False to trigger non-tiled conv
        return False, t, t_count

    return True, t, t_count
...
def conv_tile(
    ...
    split_weights,
    sparse,
    full_rows_first=False,
    partial_rows_first=False,
):
    v = lambda m, im, r, c: valid_fia(
        m, im, r, c, node, preset, use_db, row_db, split_weights, sparse=sparse
    )
    BATCH, IMAPS, IROWS, ICOLUMNS, MAPS, ROWS, COLUMNS = 0, 1, 2, 3, 4, 5, 6
    t = tile.copy()
    ...
    # start with setting tile omaps/imaps to parallel_output_maps / padded_channels
    t[MAPS] = min(node.Conv2DOptions.filter_shape_dims[0], parallel_output_maps)
    t[IMAPS] = min(node.Conv2DOptions.filter_shape_dims[1], padded_channels)
    ...
    if full_rows_first:
        ...
        while limit_rows > t[ROWS] and not v(t[MAPS], t[IMAPS], limit_rows, t[COLUMNS]):
            limit_rows -= 1
        t = increment_tile(
            node, preset, sp, fx, t, ROWS, limit=limit_rows, sparse=sparse
        )

    elif partial_rows_first:
        limit_rows = min(node.m, t[ROWS] + dkh - 1)
        while limit_rows < node.m and not v(t[MAPS], t[IMAPS], limit_rows, t[COLUMNS]):
            limit_rows += 1
        if v(t[MAPS], t[IMAPS], limit_rows, t[COLUMNS]):
            t = increment_tile(
                node, preset, sp, fx, t, ROWS, limit=limit_rows, sparse=sparse
            )
    ...
    # max imaps
    limit_imaps = int(
        np.ceil(node.Conv2DOptions.filter_shape_dims[1] / padded_channels)
        * padded_channels
    )
    while not v(t[MAPS], limit_imaps, t[ROWS], t[COLUMNS]):
        limit_imaps /= 2
    while limit_imaps % padded_channels != 0:
        limit_imaps -= 1
    t = increment_tile(
        node,
        preset,
    ...
    if t[IMAPS] > node.Conv2DOptions.filter_shape_dims[1]:
        t[IMAPS] = node.Conv2DOptions.filter_shape_dims[1]
    ...
    t_count = set_tile_attr(node, preset, sp, t, sparse=sparse)

    return t, t_count, v(t[MAPS], t[IMAPS], t[ROWS], t[COLUMNS])
...
def usage(node, tile, preset, verbose=False, sparse=None):
    ...
    if node.Conv2DOptions.use_depthwise:
        w_used = (
            tile[IMAPS] * kw * (kh + parallel_output_maps - 1) * parallel_output_maps
        )
    else:
        ...
        if sparse == 1:
            c_used_in_w = c_used_in_w + (
                8 - ((c_used_in_w % 8) if (c_used_in_w % 8) else 8)
            )
            if node.Conv2DOptions.repeat == 2:
                c_used_in_w //= 2
            elif node.Conv2DOptions.repeat == 1:
        ...
        k_used_in_w += parallel_output_maps - (
            (k_used_in_w % parallel_output_maps)
            if (k_used_in_w % parallel_output_maps)
            else parallel_output_maps
        )
    ...
    total_k += parallel_output_maps - (
        (k % parallel_output_maps)
        if (k % parallel_output_maps)
        else parallel_output_maps
    )
    ...
    if verbose:
        print(
            "in  {}% (used {}, size {})".format(
                int(100.0 * in_used / in_size), in_used, in_size
            )
        )
        print(
            "wt  {}% (used {}, size {}, total {})".format(
                int(100.0 * w_used / w_size), w_used, w_size, w_total
            )
        )
        print(
            "qt  {}% (used {}, size {}, total {})".format(
                int(100.0 * q_used / q_size), q_used, q_size, q_total
            )
        )
        print(
            "sp  {}% (used {}, size {}, {})".format(
                int(100.0 * sp_used / sp_size), sp_used, sp_size, (v_out, v_tmp0, v_tmp)
            )
        )
...
def tile_subgraph(
    ...
    if not node.offloaded:
        for idx in range(len(node.tensor_array)):
        ...
        for i in range(len(node.subnode_array)):
            ...
            for idx in range(len(sn.tensor_array)):
    ...
    # tile = minimum_tile_subgraph(node, fx)
    # Need to handle constraint propagation issue where minimum tile may not be valid for entire subgraph, so need to check and
    # if not valid then adjust minimum tile until valid (currently just adjusting columns since that is usually the constraining
    # dimension but could be other dimensions for different subgraphs)
    tile, allocation_succeed = minimum_valid_tile_subgraph(
        node,
        preset,
        sp,
        fx,
    ...
    # Verify the adjusted tile still fits in scratchpad
    if is_sp_invalid(node, preset, sp, tile, sparse=sparse):
        # Subnode requirements cannot be satisfied within scratchpad
        return tile, False

    min_tile = tile.copy()

    if node.type == BuiltinOperator.RESIZE_BILINEAR:
        if not (node.m == tile[ROWS] and node.n == tile[COLUMNS]) and (
            node.ResizeOptions.scale[-1] not in [2.0, 4.0, 8.0]
        ):
            node.ResizeOptions.min_rows = min_tile[-2]
            node.ResizeOptions.min_cols = min_tile[-1]
            node.ResizeOptions.min_maps = min_tile[-3]
    ...
    if e in [
        ...
        if not node.Conv2DOptions.use_fia:  # scalar + vector
            tile = increment_tile(
                node, preset, sp, fx, tile, COLUMNS, limit=node.n, sparse=sparse
            )
            tile = increment_tile(
                node, preset, sp, fx, tile, ROWS, limit=node.m, sparse=sparse
            )
            tile = increment_tile(
                node,
                preset,
                ...
                tile,
                MAPS,
                limit=node.Conv2DOptions.kernels,
                sparse=sparse,
            )
            is_allocated = True

        elif node.Conv2DOptions.use_depthwise:
            is_allocated, tile = fit_depthwise(
                tile, node, sp, fx, preset, use_db=1, row_db=0, sparse=sparse
            )
        else:  # accelerator
            is_allocated, tile, t_count = fit_conv(
                tile, node, sp, fx, preset, use_db=1, row_db=0, sparse=sparse
            )

        if not is_allocated:
            with sp_invalid_exception_catcher(
                sp, opcode, json_graph, split, graph_idx, 1
            ):
                assert 0
        ...
        if node.FullyConnectedOptions.use_fia:  #
            v = lambda a, r, c: valid_fia_fc(
                a,
                r,
                c,
                preset,
                (len(node.subnode_array) == 0 and node.output_strides == [1, 1]),
            )

            # maximize accum depth (input width, filter rows)
            limit_accum_depth = accum_depth
            while limit_accum_depth > tile[ICOLUMNS] and not v(
                limit_accum_depth, tile[ROWS], tile[COLUMNS]
            ):
                limit_accum_depth -= 1
            tile = increment_tile(
                node,
                ...
                tile,
                ICOLUMNS,
                limit=limit_accum_depth,
                sparse=sparse,
            )

            # maximize output depth (filter columns, output columns)
            limit_output_depth = output_depth
            while limit_output_depth > tile[COLUMNS] and not v(
                tile[ICOLUMNS], tile[ROWS], limit_output_depth
            ):
            ...
            tile = increment_tile(
                ...
                tile,
                COLUMNS,
                limit=limit_output_depth,
                sparse=sparse,
            )

            # maximize input rows (output rows)
            limit_rows = node.m
            while limit_rows > tile[ROWS] and not v(
                tile[ICOLUMNS], limit_rows, tile[COLUMNS]
            ):
                limit_rows -= 1
            tile = increment_tile(
                node, preset, sp, fx, tile, ROWS, limit=limit_rows, sparse=sparse
            )
        else:
            tile = increment_tile(
                node, preset, sp, fx, tile, ICOLUMNS, limit=accum_depth, sparse=sparse
            )
            tile = increment_tile(
                node, preset, sp, fx, tile, COLUMNS, limit=output_depth, sparse=sparse
            )
            tile = increment_tile(
                node, preset, sp, fx, tile, IROWS, limit=output_depth, sparse=sparse
            )
        ...
        tile = increment_tile(
            node, preset, sp, fx, tile, MAPS, limit=node.channels, sparse=sparse
        )

    # verify that final tile fits in sp, and if it is Conv additionally verify that valid_fia still true else exit
    if not is_sp_invalid(node, preset, sp, tile, sparse):
        if e in [
            BuiltinOperator.CONV_2D,
            BuiltinOperator.TRANSPOSE_CONV,
        ]:
    ...
    # # verify that final tile satisfies all subnode constraints
    if not is_tile_valid_for_subgraph(node, tile, sparse):
        return tile, False


    if e == BuiltinOperator.RESIZE_BILINEAR and 0:
        # we perform sequential loads from the minimum tile for non-2.0/4.0 scales
        # not full maps and enough rows or cols that matches VECTOR LANES
        if (
            (node.ResizeOptions.scale[-1] not in [2.0, 4.0, 8.0])
            and not (node.m == tile[ROWS] and node.n == tile[COLUMNS])
            and (not min_valid_tile)  # TODO min_valid_tile does not exist deprecated
        ):
            # print("non 2.0/4.0 scale and tile does not match dimensions, performing sequential loads from minimum tile", node.ResizeOptions.scale)
            min_rows, min_cols, min_maps, rows_trips, cols_trips, maps_trips = (
                plan_sequantial_loads_from_min_tile(
                    min_tile, node, preset, sp, sparse=sparse
                )
            )
            # tile[-3] = min_maps
            # tile[-2] = min_rows
            # tile[-1] = min_cols
            node.ResizeOptions.min_rows = min_rows
            node.ResizeOptions.min_cols = min_cols
            node.ResizeOptions.min_maps = min_maps
            node.ResizeOptions.rows_trips = rows_trips
            node.ResizeOptions.cols_trips = cols_trips
            node.ResizeOptions.maps_trips = maps_trips
            node.ResizeOptions.in_scratchpad = 0
        elif not (node.m == tile[ROWS] and node.n == tile[COLUMNS]) and (
            node.ResizeOptions.scale[-1] not in [2.0, 4.0, 8.0]
        ):
            node.ResizeOptions.min_rows = min_tile[ROWS]
            node.ResizeOptions.min_cols = min_tile[COLUMNS]
            node.ResizeOptions.min_maps = min_tile[MAPS]
            node.ResizeOptions.rows_trips = tile[ROWS] // min_tile[ROWS]
            node.ResizeOptions.cols_trips = tile[COLUMNS] // min_tile[COLUMNS]
            node.ResizeOptions.maps_trips = tile[MAPS] // min_tile[MAPS]
            node.ResizeOptions.in_scratchpad = 0
        elif node.ResizeOptions.scale[-1] in [2.0, 4.0, 8.0] and not (
            node.m == tile[ROWS] and node.n == tile[COLUMNS]
        ):
            node.ResizeOptions.min_rows = tile[ROWS]
            node.ResizeOptions.min_cols = tile[COLUMNS]
            node.ResizeOptions.min_maps = tile[MAPS]
            node.ResizeOptions.rows_trips = 1
            node.ResizeOptions.cols_trips = 1
            node.ResizeOptions.maps_trips = 1
            node.ResizeOptions.in_scratchpad = 0
        else:
            node.ResizeOptions.min_rows = tile[ROWS]
            node.ResizeOptions.min_cols = tile[COLUMNS]
            ...
            node.ResizeOptions.cols_trips = 1
            node.ResizeOptions.maps_trips = 1
            node.ResizeOptions.in_scratchpad = 1

  
    # STEP 3: for a given tile, determine how to walk vnnx_graph
    tile_count = set_tile_attr(node, preset, sp, tile, sparse=sparse)
    # pprint_tile(node, tile)
    # print(tile, tile_count)
