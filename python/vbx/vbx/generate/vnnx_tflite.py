...
import sys
import os.path
from math import floor, ceil, frexp, copysign, sqrt
...
from .vnnx_allocation import (
    fia_weight_shaper_size_kb,
    tile_subgraph,
    fia_weight_shaper_size_per_bank_kb,
    get_output_shapes,
    WEIGHT_SHAPER_DATA_BANKS,
)
from .utils import (
    compress1,
    compress2,
    compress4,
    json_load,
    sp_invalid_exception_catcher,
)
from .split_tflite import is_split_idx_in_engine_graphs
from .transform_tflite import (
    graph_pattern,
    lut_i8_to_u8,
    channels_first_shape,
    create_graph_from_split,
)
...
class NodeProcessingState:
    """Shared mutable state passed through node processing functions."""

    splits: List
    preset: Any
    weights: Any  # bytearray, mutable
    aliased_ids: Dict
    ids_with_dummies: Any
    in_id_to_out_ids: Dict
    tmp_dir: Any = None
    tmp_dir_obj: Any = None
    compression_vbx: Any = None
    engine_graphs_nx: Any = None
    external_inputs: Any = None
    external_outputs: Any = None
    json_graph: Any = None  # Full JSON graph for saving failing subgraphs
    Nodes: List = field(default_factory=list)
    Nodes_to_cuts_dict: Dict = field(default_factory=dict)
    dequantize_op_params: Dict = field(default_factory=dict)
    already_external_producer: Dict = field(default_factory=dict)
    already_external_consumer: Dict = field(default_factory=dict)
    first_fia: bool = False
    prev_fia_node: Optional[Any] = None
    nlf_data: Optional[Any] = None
    lut_ops: Optional[Any] = None
    reshape_ops: Optional[Any] = None
...
def compress_weights(weights):
    ...
    while True:
        for block in range(num_blocks):
            weight_block = weights[
                block * UNCOMPRESSED_BLOCK_SIZE : (block + 1) * UNCOMPRESSED_BLOCK_SIZE
            ]
...
def pack_compressed_weights(
    weights_comp, ctrl_comp, R, tile_channels, tile_maps, fit_weights=0
):
    """
    ...
    for chunk in range(channel_chunks):
        bank0 = weights_comp[
            ...
            :,
            :,
            (chunk * num_paired_channels * 2) : (chunk + 1)
            * num_paired_channels
            * 2 : 2,
        ].transpose((0, 3, 1, 2))
        bank1 = weights_comp[
            ...
            :,
            :,
            (chunk * num_paired_channels * 2)
            + 1 : (chunk + 1) * num_paired_channels * 2 : 2,
        ].transpose((0, 3, 1, 2))
        chunk_ctrl = ctrl_comp[
            :, :, :, (chunk * num_paired_channels) : (chunk + 1) * num_paired_channels
        ].transpose((0, 3, 1, 2))
        ...
        if chunk == 0:
            ...
            rechunked_weights = np.concatenate(
                (rechunked_weights, chunk_weights), axis=1
            )
    ...
    bank_chunk_size = (
        num_paired_channels * tile_maps * h * w
    )  # expected size for each bank per (kernel,channel) chunk
    ...
    if fit_weights == 1:
        ...
        for ke in range(kernel_chunks):
            ...
            if (
                ke == kernel_chunks - 1
            ):  # adjust for final tile maps which may be different
            ...
            for ch in range(channel_chunks):
                if (
                    ch == channel_chunks - 1
                ):  # adjust for final tile channels which may be different
                    final_tile_channels = (
                        (c % tile_channels) if (c % tile_channels) else tile_channels
                    )
                    final_tile_num_paired_channels = R * (final_tile_channels // 8)
                    bank_chunk_size = (
                        final_tile_num_paired_channels * final_tile_maps * h * w
                    )
                    ...
                    bank0_chunk[chunk_start:chunk_end] = (
                        weights_comp[
                            ke * tile_maps : (ke + 1) * tile_maps,
                            (bank_channel_chunk_offset + 0)
                            * num_paired_channels : (
                                (bank_channel_chunk_offset + 0) * num_paired_channels
                            )
                            + final_tile_num_paired_channels,
                            :,
                            :,
                        ]
                        .transpose((1, 2, 3, 0))
                    ...
                    bank1_chunk[chunk_start:chunk_end] = (
                        weights_comp[
                            ...
                            + (2 * final_tile_num_paired_channels),
                            :,
                            :,
                        ]
                        .transpose((1, 2, 3, 0))
                    ...
                    bank0_chunk[chunk_start:chunk_end] = (
                        weights_comp[
                            ke * tile_maps : (ke + 1) * tile_maps,
                            (bank_channel_chunk_offset + 0)
                            * num_paired_channels : (bank_channel_chunk_offset + 1)
                            * num_paired_channels,
                            :,
                            :,
                        ]
                    ...
                    bank1_chunk[chunk_start:chunk_end] = (
                        weights_comp[
                            ke * tile_maps : (ke + 1) * tile_maps,
                            (bank_channel_chunk_offset + 1)
                            * num_paired_channels : (bank_channel_chunk_offset + 2)
                            * num_paired_channels,
                            :,
                            :,
                        ]
                ...
                ctrl_chunk[chunk_start:chunk_end] = (
                    ctrl_comp[
                        ke * tile_maps : (ke + 1) * tile_maps,
                        ch * num_paired_channels : (ch + 1) * num_paired_channels,
                        :,
                        :,
                    ]
                    .transpose((1, 2, 3, 0))
        ...
        for ke in range(kernel_chunks):
            ...
            for ch in range(channel_chunks):
                ...
                data_chunk = weights_comp[
                    ke * tile_maps : (ke + 1) * tile_maps,
                    ch * num_paired_channels * 2 : (ch + 1) * num_paired_channels * 2,
                    :,
                    :,
                ].transpose((1, 2, 3, 0))
                ctrl_chunk = ctrl_comp[
                    ke * tile_maps : (ke + 1) * tile_maps,
                    ch * num_paired_channels : (ch + 1) * num_paired_channels,
                    :,
                    :,
                ...
                flat[flat_start:flat_end] = np.concatenate(
                    (data_chunk.flatten(), ctrl_chunk.flatten()), axis=-1
                )
                flat_start = flat_end
...
def pad_weights_and_pack_2_1(
    filter_data,
    node,
    parallel_output_maps,
    tile,
    preset,
    opcode,
    weight_pad=0,
    ...
    _, IMAPS, _, _, MAPS, _, _ = 0, 1, 2, 3, 4, 5, 6
    ...
    if node.type == BuiltinOperator.FULLY_CONNECTED:
        ...
        for i in range(o_):
            ...
            pad_chunk = (
                padded_data[i * node.cols : (i + 1) * node.cols]
                .transpose((1, 0))
                .flatten()
            )
            flat[2 * i * node.cols * a : 2 * (i * node.cols + f.shape[0]) * a] = (
                np.concatenate((f_, pad_chunk), axis=-1)
            )
        filter_data = flat
        ...
        )  # re-tile in case of dilation (weights are padded, and tile will affect this packing)
        ...
        if (
            tile[IMAPS] == kmaps
        ):  # the tile fits all imaps/maps, so only one extra chunk is needed (additional DMAs will be dummies)
            padded_kmaps = kmaps + tile[IMAPS]
        if sparse == 1:
            chunk_of_zeros = np.zeros(tile[IMAPS] * col_h_out * col_w_out * kw)
            filler_chunk = np.concatenate(
                (
                    chunk_of_zeros,
                    np.full((tile[IMAPS] * col_h_out * col_w_out * kw), 64),
                )
            )
        ...
        padded_weights = np.zeros(
            np.prod((padded_kmaps, col_h_out * col_w_out, kw)), dtype=np.int8
        )
        ...
        while kmap < padded_kmaps:
            ...
            for col in range(kw):
                ...
                padded_weights[
                    ((kmap * kw) + col)
                    * col_chunk_size : ((kmap * kw) + col + 1)
                    * col_chunk_size
                ] = toeplitz(c, zero).flatten()
            kmap += 1
            filter_data_kmap += 1

            # pad the weights with the filler chunk for the first two chunks (buffers, tiles etc)
            if (
                filter_data_kmap % tile[IMAPS] == 0
                or filter_data_kmap % tile[MAPS] == 0
            ) and chunks_padded < 2:
                if sparse == 1:
                    padded_weights[
                        (kmap * kw * col_chunk_size) : (
                            (kmap + (2 * tile[IMAPS])) * kw * col_chunk_size
                        )
                    ] = filler_chunk
                    kmap += 2 * tile[IMAPS]
                else:
                    padded_weights[
                        (kmap * kw * col_chunk_size) : (
                            (kmap + tile[IMAPS]) * kw * col_chunk_size
                        )
                    ] = filler_chunk
                    kmap += tile[IMAPS]
                chunks_padded += 1
        ...
        if not conv8.use_depthwise:
            if (sparse == 1) and kc % 8 != 0:
                ...
                filter_data = np.pad(
                    filter_data,
                    pad_width=((0, 0), (0, channel_pad), (0, 0), (0, 0)),
                    mode="constant",
                    constant_values=0,
                )
            elif kc % 2 != 0:
            ...
            if (kmaps % parallel_output_maps) != 0:
                ...
                filter_data = np.pad(
                    filter_data,
                    pad_width=((0, padding), (0, 0), (0, 0), (0, 0)),
                    mode="constant",
                    constant_values=0,
                )
            ...
            if sparse == 1:
                ...
                # debug print to find out the block compression determined at compile level
                # formatted as layer_name, input_shape, kernel_shape, block_compression
                if verbose:
                    bl_comp = 1
                    if R == 2:
                        bl_comp = 2
                    ...
                    print(
                        "Conv_{}x{}{}".format(
                            ...
                            kw,
                            (
                                "_str{}".format(conv8.stride_width)
                                if conv8.stride_width != 1
                                else ""
                            ),
                        ),
                        end="\t",
                    )
                    print("[{},{},{}]".format(node.channels, node.m, node.n), end="\t")
                    print(conv8.filter_shape_dims, end="\t")
                    print(bl_comp)
                ...
                if (
                    conv8.fit_weights == 1
                    and ALL_WEIGHTS
                    and (np.prod(weights_comp.shape) + np.prod(ctrl_comp.shape))
                    > (fia_weight_shaper_size_kb(parallel_output_maps) * 1024)
                ):
                    conv8.fit_weights = 0
                ...
                if retiled:
                    repack_maps = node.maps
                    if node.maps == kmaps:  # tile is doing all omaps
                        repack_maps = filter_data.shape[
                            0
                        ]  # this is the padded kernel value

                filter_data = pack_compressed_weights(
                    weights_comp,
                    ctrl_comp,
                    R,
                    tile[IMAPS],
                ...
                if is_transpose:
                    filter_data = conv_pack_weights(
                        filter_data, repack_maps, tile[IMAPS], is_transpose=True
                    )
                else:
                    ...
                    if repack_maps == 0:
                        ...
                        print(
                            "[{},{},{}]".format(node.channels, node.m, node.n), end="\t"
                        )
                        print(conv8.filter_shape_dims, end="\t")
                    filter_data = conv_pack_weights(
                        filter_data, repack_maps, tile[IMAPS]
                    )
...
def exception_catcher(
    node, node_index, tensor_index=None, subnode=None, subnode_index=None
):
    try:
        yield
...
def LUTPopulateInt8(
    ...
    itype=calc_type.INT8,
    otype=calc_type.INT8,
):
    ...
    for idx in range(min_idx, max_idx + 1):
        ...
        lut_uint8[np.int32(idx).astype(np.uint8)] = np.int32(
            max(min(max_out, quantized), min_out)
        ).astype(np.uint8)
...
def LUTPopulate(
    ...
    for idx in range(min_idx, max_idx + 1):
        ...
        if not tf.math.is_nan(transformed) and not tf.math.is_inf(transformed):
            rescaled = TfLiteRound(transformed * inverse_scale)
            quantized = rescaled + output_zero_point
        lut_uint[np.int32(idx).astype(np.uint8)] = np.int32(
            max(min(max_val, quantized), min_val)
        ).astype(lut_dtype)
...
def conv_pack_shift(data, maps):
    ...
    for k in range(ceil(kernels / maps)):
        ...
        packed[k * maps : (k + 1) * maps] = np.frombuffer(
            words.tobytes(), dtype=np.int32
        )
...
def conv_pack_biases(data, maps):
    ...
    for k in range(ceil(kernels / maps)):
        bytes = np.frombuffer(
            flat[k * maps : (k + 1) * maps].flatten().tobytes(), dtype=np.uint8
        )
        shuffled = bytes.reshape((-1, 4)).transpose((1, 0)).flatten()
        packed[k * maps : (k + 1) * maps] = np.frombuffer(
            shuffled.tobytes(), dtype=np.int32
        )

    return packed.tolist()


def conv_pack_weights(
    data, maps, tile_channels, use_depthwise=False, is_transpose=False, weight_pad=0
):
    # k,c,h,w -> c,h,w,k
    s = data.shape
    t = (1, 2, 3, 0)
    ...
    if s[0] == 1 and not is_transpose:
        ...
        if use_depthwise:
            ...
            new_data_shape = (
                ...
                kernels,
                kernel_width,
                kernel_height + (2 * weight_pad),
            )
            chunk_size = new_data_shape[2] * new_data_shape[3]
            data = np.pad(
                data,
                ((0, 0), (weight_pad, weight_pad), (0, 0)),
                mode="constant",
                constant_values=0,
            )
...
def quantize_two_math_block(
    ...
    if (
        len(effective_output_scale) < len(bias_data)
        and len(effective_output_scale) == 1
    ):  # FullyConnected
    ...
    for bias, scale in zip(bias_data, effective_output_scale):
        ...
        expected_saturation = (
            max_val < output_activation_min and min_val < output_activation_min
        ) or (max_val > output_activation_max and min_val > output_activation_max)
        ...
        if scale == 0:
            ...
            while round(om) > 2**OM_BITS - 1:
                ...
                m_shift_min += 1
        while 1:
            indices = np.where(np.array(O_SHIFT_RANGE) <= -m_shift_min)[
                0
            ]  # o_shift = -m_shift
            ind = 0
            if len(indices) > 0:
            ...
            om, om_sat = saturate(
                int(round(scale * 2.0**-m_shift)), 0, 2 ** (OM_BITS) - 1
            )
            if om_sat:
                assert m_shift < m_shift_min
            else:
                assert m_shift >= m_shift_min

            bias_premult = (
                bias * om
            )  # using the floating-point scale here actually has a less accurate result in many cases
            offset = output_offset << o_shift
            round_bit = 1 << (o_shift - 1)
            _, m_sat_high = saturate(
                bias_premult + offset + round_bit + int30max * om, int48min, int48max
            )
            _, m_sat_low = saturate(
                bias_premult + offset + round_bit + int30min * om, int48min, int48max
            )
            cInput, c_sat = saturate(
                bias_premult + offset + round_bit, int48min, int48max
            )
            cInputL = cInput % 65536  # 16 bits
            cInputH = (cInput - cInputL) >> 16  # 32 bits
            cInputL = 64 * floor(
                cInputL / 64.0
            )  # round to zero lowest 6 bits, which has a negligible effect on precision
...
def quantize_multiplier(double_multiplier, TFLITE_SINGLE_ROUNDING=False):
    ...
    assert (
        double_multiplier >= 0
    ), "Double multiplier of quantize_multiplier is not greater or equal to 0"
    ...
    assert q_fixed <= (
        2**QMULT
    ), "q_fixed in quantize_multiplier is not less or equal to 2^QMULT"
    if q_fixed == (2**QMULT):
        q_fixed /= 2
        shift += 1
    assert (
        q_fixed <= np.iinfo(np.int32).max
    ), "q_fized in quantize_multiplier is not less or equal to numpy int32 maximum"
...
def get_effective_quantized_multiplier_from_tensors(
    i_tensor, o_tensor, f_tensor, filter_scale=None
):
    ...
    effective_output_scale = [
        input_scale[0] * f / output_scale[0] for f in filter_scale
    ]
...
def get_effective_quantized_multiplier_from_op(op, tensors, filter_scale=None):
    ...
    if filter_scale is None:
        ...
        f_tensor = None
    return get_effective_quantized_multiplier_from_tensors(
        i_tensor, o_tensor, f_tensor, filter_scale
    )
...
def CalculateInputRadius(input_integer_bits, input_left_shift, total_signed_bits):
    return [
        ((2**input_integer_bits) - 1)
        * (2 ** (total_signed_bits - input_integer_bits))
        // (2**i)
        for i in input_left_shift
    ]
    # else:
...
def check_unpack(ishape, oshape, axis, c_axis, count):
    ...
    if c_axis == -1:
        if (
            len(ishape) >= 4
        ):  # axis = -1    NHWC -> NHC   NCHW->CNH  single col then 1,0,2
        ...
        if (
            len(ishape) >= 4
        ):  # axis = -2    NHWC -> NWC   NCHW->CNW  single row, then 1,0,2
...
def check_pack(ishape, oshape, axis, c_axis, count):
    ...
    if c_axis == -1:
        if (
            len(ishape) >= 3
        ):  # axis = -1    HWC -> HWxC   CHW->HCWx  1,0,2 then multi-col
            tmp = [
                np.transpose(_, adjust_transform([1, 0, 2], _.ndim)) for _ in c_iarrs
            ]
        ...
        if (
            len(ishape) >= 3
        ):  # axis = -2    HWC -> HxWC   CHW->HCxW  1,0,2 then multi-row
            tmp = [
                np.transpose(_, adjust_transform([1, 0, 2], _.ndim)) for _ in c_iarrs
            ]
...
def channels_first_array_reshape(ishape, transform):
    ...
    # ones = lambda x: len([_ for _ in x if _ == 1]) TODO remove once def function verified
    def ones(x):
        return len([_ for _ in x if _ == 1])

    # squeezed_shape = lambda sh, ni: tuple([_ for i, _ in enumerate(sh) if i != len(sh) + ni]) TODO remove once def function verified
    def squeezed_shape(sh, ni):
        return tuple([_ for i, _ in enumerate(sh) if i != len(sh) + ni])

    squeeze = False
    if ones(ishape) - 1 == ones(oshape) and len(ishape) - 1 == len(oshape):
        squeeze = True
    ...
    if join and idims > 3:
        ...
        if join_axis < -3:
            ...
            mode = 12 + 1
            tmp = np.transpose(
                c_iarr, adjust_transform([2, 0, 1], c_iarr.ndim)
            )  # 2,0,1   n,c,h,w -> n,w,c,h
            tshape = (
                list(tmp.shape)[:-3] + [tmp.shape[-3] * tmp.shape[-2]] + [tmp.shape[-1]]
            )
    ...
    if idims == odims:
        if (
            oshape[-3] == ishape[-1]
            and oshape[-2] == ishape[-3]
            and oshape[-1] == ishape[-2]
        ):  # swap channels first to last aka 2,0,1
            return 2
    ...
    if len(ishape) - len(oshape) > 1:
        print(
            ...
            "->",
            tuple(oshape),
            "cannot currently handle multi-axis squeeze",
        )
    if len(oshape) - len(ishape) > 1:
        print(
            ...
            "->",
            tuple(oshape),
            "cannot currently handle multi-axis expand",
        )
...
def precalculate_filter_input_bias(filters, input_offset, filter_offset=0, reduce=True):
    if filters.shape == ():  # numpy array 0-dimensional, single element filter
        return np.array(
            [(np.sum(filters) * -input_offset) + (-filter_offset * -input_offset)]
        )
    if (
        filters.shape[0] == 1 and reduce and len(filters.shape) != 1
    ):  # TODO cover when group > 1, but not depthwise
        filters = filters[0]
...
def tensor_in_id_to_out_id_mapping(json_graph, splits):
    in_id_to_out_ids = {}
    for split in splits:
        for op in [json_graph["subgraphs"][0]["operators"][_] for _ in split]:
            g_inputs = op["inputs"]
    ...
    in_id_to_out_ids = {
        key: value for key, value in in_id_to_out_ids.items() if len(value) > 1
    }
    return in_id_to_out_ids


# this function is to be called after populate_nodes and used for pad sublayer injection
# TODO This function might be deprecated and possibly need to be removed
def final_check_pad_sublayer_injection(
    in_id_to_out_ids, Nodes, ids_with_dummies, preset
):
    ...
    for in_id, out_ids in in_id_to_out_ids.items():
        ...
        for node in Nodes:
            if node.tensor_array[node.num_inputs].id == in_id:
                ...
                node.tensor_array[0].id == in_id
                and node.tensor_array[node.num_inputs].id in out_ids
            ):
        ...
        if len(dest_nodes) > 0:
            ...
            all_same_type = all(
                dnode.type == BuiltinOperator.CONV_2D for dnode in dest_nodes
            )
            all_same_input_offset = all(
                dnode.input_offset == dest_nodes[0].input_offset for dnode in dest_nodes
            )
            pad_attributes = ["padding_width", "padding_height"]
            all_same_pad_hw = all_same_type and all(
                all(
                    getattr(dnode.Conv2DOptions, attr)
                    == getattr(dest_nodes[0].Conv2DOptions, attr)
                    for attr in pad_attributes
                )
            ...
            if all_matching_attr:
                ...
                for dnode in dest_nodes:
                    pad_hw = [
                        dnode.Conv2DOptions.padding_height,
                        dnode.Conv2DOptions.padding_width,
                    ]
                    if dnode.type == BuiltinOperator.CONV_2D and (
                        dnode.Conv2DOptions.padding_height > 0
                        or dnode.Conv2DOptions.padding_width > 0
                    ):
                        # print('2 ', dnode.tensor_array[0].id)
                        Nodes = inject_dummy_identity(
                            Nodes,
                            ids_with_dummies,
                            dnode,
                            preset,
...
def write_io_offset_map(
    tmp_dir, Nodes, weights, test_inputs, test_outputs, io_vnnx_offset
):
    all_tensor_array = []
    for n in Nodes:
    ...
    with open(offset_map_fname, "w") as table:
        ...
        for name in unique_names:
            ...
            ts_io = [
                t
                for t in ts
                if (t.name in test_inputs.keys() or t.name in test_outputs.keys())
            ]
            for idx, t in enumerate(ts_io):
                ...
                table.write(
                    "\t".join([io_type, t.name, str(t.id), str(len(data)), str(offset)])
                    + "\n"
                )
                offset += (
                    np.prod(mod_shape(t.shape[:4], 4)) * 2
                )  # See set_tensor_buffer
...
def set_fia_preload(Nodes):
    ...
    for n, node in enumerate(Nodes):
        if node.type in [
            ...
            node.Conv2DOptions.preload_next_fia = 0
            node.Conv2DOptions.next_fia_node_id = -1

            if prev_fia_node:
                prev_fia_node.Conv2DOptions.next_fia_node_id = n
            prev_fia_node = node
    ...
    for n, node in enumerate(Nodes):
        if node.type in [
            ...
            fia_count += 1
            if node.Conv2DOptions.next_fia_node_id != -1:
                nn, next_node = (
                    node.Conv2DOptions.next_fia_node_id,
                    Nodes[node.Conv2DOptions.next_fia_node_id],
                )
                ...
                for p, _ in prev_nodes(next_node, Nodes):
                    if p > nn_prev:
                        nn_prev = p
                if nn_prev != -1:
                    # case 1 preload, prior parent node
                    if nn_prev < n:
                        next_node.Conv2DOptions.fia_preloaded = 1
                        node.Conv2DOptions.preload_next_fia = 1
                    elif nn_prev == n:
                        last_maps = node.Conv2DOptions.kernels
                        while last_maps - node.maps > 0:
                            last_maps -= node.maps

                        # case 2 preload, prior maps
                        if (
                            node.Conv2DOptions.kernels - (last_maps + node.maps)
                        ) > next_node.Conv2DOptions.imaps:
                            # 2nd input tile of next_node need also check against last output tile of node
                            if (node.Conv2DOptions.kernels - last_maps) > min(
                                next_node.Conv2DOptions.imaps * 2, next_node.channels
                            ):
                                next_node.Conv2DOptions.fia_preloaded = 1
                                node.Conv2DOptions.preload_next_fia = 1

                        # case 3 preload, prior rows
                        if (node.orow_last - node.row_inc) > (
                            next_node.row_start + next_node.rows_0
                        ):
                            # 2nd input tile of next_node need also check against last output tile of node
                            if node.orow_last > min(
                                next_node.row_start + next_node.rows_0 + next_node.rows,
                                next_node.m,
                            ):
                                next_node.Conv2DOptions.fia_preloaded = 1
                                node.Conv2DOptions.preload_next_fia = 1
        ...
        for n, node in enumerate(Nodes):
            if node.type in [
                ...
                if node.Conv2DOptions.fia_preloaded:
                    valid_count += 1
                if node.Conv2DOptions.preload_next_fia:
                    next_preload_count += 1
        print("inputs preloadable", valid_count, "/", fia_count)
        print(
            "next preloadable", next_preload_count, "/", fia_count
        )  # should be equal to inputs preloadable
...
def generate_vnnx_from_json_subgraphs(
    json_graph,
    splits,
    activations,
    preset,
    ...
    if engine_graphs_nx:
        external_inputs, external_outputs = get_external_io(
            json_subgraphs_from_splits(json_graph, splits), engine_graphs_nx
        )

    in_id_to_out_ids = tensor_in_id_to_out_id_mapping(json_graph, splits)
    ...
    # TODO
    # final_check_pad_sublayer_injection(in_id_to_out_ids, Nodes, ids_with_dummies, preset)
    ...
    io_nodes, io_offsets = set_io_nodes(
        vnnx_graph, Nodes, graph_inputs, graph_outputs, weights
    )
    ...
    if compression_vbx == "ucomp":
        act_buffer_size, io_buffer_size = set_io_buffers(
            Nodes, weights, test_inputs, test_outputs
        )
    else:
        act_buffer_size, io_buffer_size = set_io_buffers_reused(
            Nodes, weights, test_inputs, test_outputs
        )
    ...
    replay_start = len(
        b"".join(
            graph_data + node_data + subnode_data + tensor_data + align1 + [weights]
        )
    )
    ...
    try:
        ...
        replay_size = int.from_bytes(
            m.model_bytes[4 * (14 + 4) : 4 * (14 + 1 + 4)], byteorder="little"
        )
        vnnx_graph.replay_buffer_size = replay_size

        if not replay_first:
            replay = m.model_bytes[
                replay_start : replay_start + vnnx_graph.replay_buffer_size
            ]
            vnnx_graph.fixed_replay_buffer0 = int.from_bytes(
                m.model_bytes[4 * (4) : 4 * (4 + 1)], byteorder="little"
            )
            vnnx_graph.fixed_replay_buffer1 = int.from_bytes(
                m.model_bytes[4 * (5) : 4 * (5 + 1)], byteorder="little"
            )
            vnnx_graph.fixed_replay_buffer2 = int.from_bytes(
                m.model_bytes[4 * (6) : 4 * (6 + 1)], byteorder="little"
            )
            vnnx_graph.fixed_replay_buffer3 = int.from_bytes(
                m.model_bytes[4 * (7) : 4 * (7 + 1)], byteorder="little"
            )
        else:
            vnnx_graph.replay_buffer_size += 128
            replay = bytearray(vnnx_graph.replay_buffer_size)
            replay_start = len(
                b"".join(graph_data + node_data + subnode_data + tensor_data + align1)
            )
            vnnx_graph.replay_buffer = replay_start

            node_data, subnode_data, tensor_data, align2 = update_offsets(
                vnnx_graph,
                Nodes,
                vnnx_graph.replay_buffer_size,
                vnnx_graph.replay_buffer,
            )
            vnnx_graph.replay_buffer = replay_start
            graph_data = [vnnx_graph.get_structured_data()]
            data = b"".join(
    ...
    if debug and (compression_vbx != "ucomp"):
        ...
        print(
            "WEIGHTS - BUFFERS {:3.2f}".format(
                (len(b"".join([weights])) - (act_buffer_size + io_buffer_size)) / 2**20
            )
        )
...
def set_concat_tensor_buffer(
    tname, all_tensor_array, weights, concat_map, concat_offset
):
    ...
    if t.buffer[1] == -1:
        if t.name in concat_map.keys():
            ...
            if t_.buffer[1] == -1:
                size = (
                    np.prod(mod_shape(t_.shape, 4)) * 2
                )  # TODO fix for proper sized maps (w/ output_strides)
...
def set_tensor_buffer(idx, t, ts, weights, test_inputs, test_outputs):
    if t.buffer[1] == -1:
        ...
        if t.buffer[1] == -1:
            size = (
                np.prod(mod_shape(t.shape, 4)) * 2 * sizeof_calc_type(t.type)
            )  # TODO fix for proper sized maps (w/ output_strides)
...
def set_skip_concat(Nodes, test_inputs, test_outputs, external_inputs):
    for n, node in enumerate(Nodes):
        if node.type == BuiltinOperator.CONCATENATION:
            concat_io = [
                t
                for t in node.tensor_array
                if (t.name in test_inputs.keys() or t.name in test_outputs.keys())
            ]
            idims = len(node.tensor_array[0].shape)

            if (
                (node.ConcatOptions.axis - idims == -3)
                and len(node.subnode_array) == 0
                and len(concat_io) == 0
            ):
                node.skip = 1
                ...
                for _, prev in prev_nodes(node, Nodes):
                    shared_nodes = [
                        n_ for _, n_ in next_nodes(prev, Nodes) if n_ != node
                    ]
...
def set_skip_channel_split(Nodes, test_inputs, test_outputs, external_inputs):
    for n, node in enumerate(Nodes):
        if node.type == VNNXOperator.IDENTITY:
            if (
                len(node.subnode_array) > 0
                and node.subnode_array[0].type == BuiltinOperator.SLICE
            ):
                split_io = [
                    t for t in node.tensor_array if (t.name in test_outputs.keys())
                ]
                sn = node.subnode_array[0]
                if (
                    len(sn.tensor_array[0].shape) >= 3
                    and len(sn.tensor_array[1].shape) >= 3
                ):
                    width_matches = (
                        sn.tensor_array[0].shape[-1] == sn.tensor_array[1].shape[-1]
                    )
                    height_matches = (
                        sn.tensor_array[0].shape[-2] == sn.tensor_array[1].shape[-2]
                    )
                    channel_matches = (
                        sn.tensor_array[0].shape[-3] == sn.tensor_array[1].shape[-3]
                    )
                    is_channel_slice = (
                        width_matches and height_matches and not channel_matches
                    )

                    if is_channel_slice and len(split_io) == 0:
                        if not any(
                            [
                                n_.type == BuiltinOperator.CONCATENATION
                                for _, n_ in next_nodes(node, Nodes)
                            ]
                        ):
                            node.skip = 1
...
def shared_buffers(nodes, inputs, outputs, concat_map):
    ...
    for n, node in enumerate(nodes):
        for t in nodes[n].tensor_array:
            ...
            if t.buffer[1] != -1:
                ...
                io_size[t.name] = np.prod(mod_shape(t.shape, 4)) * sizeof_calc_type(
                    t.type
                )
    ...
    for n, node in enumerate(nodes):
        for t in nodes[n].tensor_array:
            if t.name in concat_map:
                t = find_tensors_by_name(all_tensor_array, concat_map[t.name])[0]
            if t.name not in io_names:
                if t.name not in alloc:
                    alloc[t.name] = n  # allocate only on the first use
                dealloc[t.name] = (
                    n  # deallocate on the last use; don't deallocate inputs/outputs
                )

    heap = [
        (0, 0)
    ]  # occupied memory; each list element is the range of an allocation; start with dummy node
    heap_ptr = (
        dict()
    )  # pointers to heap, including deallocated; heap_ptr[io_id] = memory location
        ...
        for u, alloc_ind in alloc.items():
            if n == alloc_ind:  # find all allocations during this layer
                for h in range(1, len(heap)):
                    if (
                        heap[h][0] - heap[h - 1][1] >= io_size[u]
                    ):  # see if new buffer can fit between allocations
                        heap_ptr[u] = heap[h - 1][1]
                        heap.insert(h, (heap_ptr[u], heap_ptr[u] + io_size[u]))
                        break
                if u not in heap_ptr:  # put new buffer at the end of the heap
                    heap_ptr[u] = heap[-1][1]
    ...
    for name in remap.keys():
        if filter is None or name in filter:
            ...
            for t in ts:
                ...
                t.buffer[1], t.direct = (
                    t_.buffer[1] + offset[t.name],
                    t_.direct + offset[t.name],
                )
...
def map_all_tensor_buffers(tarray, weights, names):
    for name in names:
        ...
        if any(_.buffer[1] == -1 for _ in ts):
            size = np.prod(mod_shape(ts[0].shape, 4)) * sizeof_calc_type(
                ts[0].type
            )  # TODO fix for proper sized maps (w/ output_strides)
            for t in ts:
...
def set_io_buffers_reused(Nodes, weights, test_inputs, test_outputs):
    ...
    remap_all_tensor_buffers(
        all_tensor_array, weights, concat_map, concat_offset, io_names
    )
...
def set_io_buffers(Nodes, weights, test_inputs, test_outputs):
    ...
    for name in unique_names:
        ...
        for idx, t in enumerate(ts_io):
            set_concat_tensor_buffer(
                t.name, all_tensor_array, weights, concat_map, concat_offset
            )
        ...
        for idx, t in enumerate(ts_in):
            print(
                "INPUT {}\n\tNAME {}\n\tSHAPE {}\n\tLENGTH {}\n\tDIRECT {} ({})\n\tIO NODE {}\n\tIO OFFSET {}".format(
                    ...
                    t.name,
                    t.shape,
                    np.prod([_ for _ in t.shape if _ > 0]),
                    t.buffer[1],
                    hex(t.buffer[1]),
                    io_nodes[io_idx],
                    io_offsets[io_idx],
...
def set_io_nodes(vnnx_graph, Nodes, graph_inputs, graph_outputs, weights):
    ...
    for i in graph_inputs + graph_outputs:
        ...
        if isinstance(Nodes[n].tensor_array[off].scale, list):
            Nodes[n].tensor_array[off].scale_f16 = [
                int(_ * 2**16) for _ in Nodes[n].tensor_array[off].scale
            ]
        else:
            Nodes[n].tensor_array[off].scale_f16 = int(
                Nodes[n].tensor_array[off].scale * 2**16
            )
    ...
    for i in range(reference_node.num_inputs):
        ...
        if pad_hw is not None:
            ...
            new_h = (
                reference_shape[-2]
                + pad_h
                + ((transpose_dilate[0] - 1) * (reference_shape[-2] - 1))
            )
            new_w = (
                reference_shape[-1]
                + pad_w
                + ((transpose_dilate[1] - 1) * (reference_shape[-1] - 1))
            )
            reference_shape = tuple(list(reference_shape)[:-2] + [new_h, new_w])
            output_shape = reference_shape
            reference_node.m, reference_node.n = (
                reference_shape[-2],
                reference_shape[-1],
            )
...
def inject_output_shaper(
    conv_node, conv_o_tensor, subnode_tensor_array, ids_with_dummies, debug=0
):
    sn = Subnode()
    ...
    oh, ow, osh, osw = get_output_shapes(
        conv_node.Conv2DOptions, conv_node.rows_0, conv_node.cols_0
    )
    ohf, owf, oshf, oswf = get_output_shapes(
        conv_node.Conv2DOptions, conv_node.rows_final, conv_node.cols_final
    )
    if debug:
        print(
            (oh, ow, osh, osw),
            (ohf, owf, oshf, oswf),
            conv_node.Conv2DOptions.use_strided,
            conv_node.Conv2DOptions.stride_height,
        )
    ...
    sn.ShaperOptions.width_final = owf
    sn.ShaperOptions.shaper_width_final = oswf
    ...
    _, _, osh, osw = get_output_shapes(
        conv_node.Conv2DOptions, conv_node.m, conv_node.n
    )
    adjusted_conv_o_tensor_shape = (
        conv_o_tensor.shape[0],
        conv_o_tensor.shape[1],
        osh,
        osw,
    )
...
def inject_strided_slice(
    conv_node, conv_o_tensor, subnode_tensor_array, ids_with_dummies
):
    sn = Subnode()
    ...
    adjusted_conv_o_tensor_width = (
        conv_in_w + pad_w - (((filter_w - 1) * dilation_w) + 1) + 1
    )
    adjusted_conv_o_tensor_shape = (
        conv_o_tensor.shape[0],
        conv_o_tensor.shape[1],
        conv_o_tensor.shape[2],
        adjusted_conv_o_tensor_width,
    )
...
def inject_pad_subnode_to_previous_node(
    prev_node,
    nodes,
    ids_with_dummies,
    preset,
    ...
    new_h = (
        input_shape[-2] + pad_h + ((transpose_dilate[0] - 1) * (input_shape[-2] - 1))
    )
    new_w = (
        input_shape[-1] + pad_w + ((transpose_dilate[1] - 1) * (input_shape[-1] - 1))
    )
...
#     if (
#         conv8.use_fia and conv8.use_depthwise and node.m == rows and node.n == columns
#     ):  # check if full maps, if so pad weights here for 1D DMA
...
#     elif (
#         conv8.use_fia and conv8.use_depthwise and (node.m != rows or node.n != columns)
#     ):
#         conv8.fit_weights = 0
...
#     conv8.repeat = -1
#     filter_data = pad_weights_and_pack_2_1(
#         filter_data,
#         node,
#         parallel_output_maps,
#         tile,
...
def json_subgraphs_from_splits(json_graph, splits):
    json_subgraphs = []
    for split in splits:
        json_subgraphs.append(create_graph_from_split(json_graph, split))
...
def add_external_producer_consumer_info(
    t,
    external_inputs,
    external_outputs,
    ...
    already_marked_external_producer = (
        t.id in already_external_producer
        and already_external_producer[t.id] != subgraph_idx
    )
    already_marked_external_consumer = (
        t.id in already_external_consumer
        and already_external_consumer[t.id] != subgraph_idx
    )
            ...
            for prev_node in Nodes:
                for tensor in prev_node.tensor_array:
                    if tensor.id == input_id:
                        if (
                            prev_node.Conv2DOptions.use_fia == 1
                            or prev_node.FullyConnectedOptions.use_fia == 1
                        ):
...
def get_quantization_params_from_tensor(node, tensors, op=None, buffers=None):
    ...
    input_scale = 1.0
    if (
        "quantization" in i_tensor.keys()
        and "zero_point" in i_tensor["quantization"].keys()
    ):
        input_offset = i_tensor["quantization"]["zero_point"][0]
    ...
    output_scale = 1.0
    if (
        "quantization" in o_tensor.keys()
        and "zero_point" in o_tensor["quantization"].keys()
    ):
    ...
    return input_offset, input_scale, output_offset, output_scale, i_tensor, o_tensor


def get_subgraph_input_names(graph):
    subgraph = graph['subgraphs'][0]
    tensors = subgraph['tensors']
    buffers = graph['buffers']

    if 'signature_defs' in graph and graph['signature_defs']:
        input_indices = [
            x['tensor_index']
            for x in graph['signature_defs'][0].get('inputs', [])
        ]
    else:
        input_indices = subgraph.get('inputs', [])

    names = set()

    for tensor_idx in input_indices:
        if tensor_idx == -1:
            continue

        tensor = tensors[tensor_idx]
        if not tensor or 'buffer' not in tensor:
            continue

        if 'data' in buffers[tensor['buffer']]:
            continue

        names.add(tensor.get('name'))

    return names


def build_subgraph_input_name_consumers(state, json_graph):
    g = 0
    consumers = {}
    while g < len(state.splits):
        graph = create_graph_from_split(json_graph, state.splits[g])
        for name in get_subgraph_input_names(graph):
            consumers.setdefault(name, []).append(g)
        g += 1

    return consumers


def _process_conv2d_node(g, node, op, opts, tensors, opcode, buffers, state, json_graph):
    """
    Extracted handling for node.type == BuiltinOperator.CONV_2D.
    Kept behavior identical to original block; returns updated (node, allocation_succeed).
    """
    conv8 = node.Conv2DOptions
    conv8.direct_dma = 0
    ...
    input_offset, input_scale, output_offset, output_scale, i_tensor, o_tensor = (
        get_quantization_params_from_tensor(node, tensors, op, buffers)
    )
    ...
    # else:
    parallel_output_maps = preset_select["PARALLEL_OUTPUT_MAPS"][state.preset]
    if k * h * w * c <= (
        (fia_weight_shaper_size_per_bank_kb(parallel_output_maps) * 1024)
        * WEIGHT_SHAPER_DATA_BANKS
    ):
    ...
    if opts.get("fused_activation_function", None) == "RELU":
        node.activation_min = output_offset
    elif opts.get("fused_activation_function", None) == "RELU6":
        node.activation_min = output_offset
    ...
    # padding (if applicable) always explicitly injected
    conv8.padding_width = 0
    conv8.padding_height = 0

    # identity should only be injected if it is strided FIA and
    # (1) there's no previous subgraph to re-map the inputs, or
    # (2) if the prev subgraph's output is mapped to different subgraphs that do not want re-mapped inputs
    inject_identity = False

    # find previous node
    pnodes = []
    prev_node = None
    prev = list(prev_nodes(node, state.Nodes))
    pnodes_idx, pnodes = zip(*prev) if prev else ((), ())
    assert(len(state.Nodes)==0 or len(pnodes) == 1) # len(pnodes)==1 for conv unless it is the 1st node (then len(state.Nodes) == 0)
    if len(pnodes) != 0:
        prev_node = pnodes[0]

    if conv8.use_strided:
        if prev_node is None: # should only true when Conv is the 1st node, which can happen for unit tests
            inject_identity = True
        else:
            # get input name of input tensor and match against all other subgraphs
            # if more than one consumer for the input tensor, then identity need to be injected
            # TODO if the other node(s) that consumes the input is strided as well, then an identity need not be injected
            current_input_tensor_idx = op['inputs'][0]
            current_input_name = tensors[current_input_tensor_idx]['name']

            subgraph_input_name_consumers = build_subgraph_input_name_consumers(state, json_graph)
            consumer_graphs = subgraph_input_name_consumers.get(current_input_name, [])

            if len(consumer_graphs) > 1:
                inject_identity = True
    ...
    if inject_identity:
        with exception_catcher(node.type, len(state.Nodes)):
            prev_len = len(state.Nodes)
            state.Nodes = inject_dummy_identity(
                state.Nodes,
                state.ids_with_dummies,
                node,
                state.preset,
                opcode,
                pad_hw=None,
                inject_strided=conv8.use_strided,
                sparse=state.compression_vbx,
                tmp_dir=state.tmp_dir,
                graph_idx=g,
                tmp_dir_obj=state.tmp_dir_obj,
            )
            # for i in range(len(state.Nodes)-prev_len):
            #     node_ids.append([-1])
    ...
    if (
        (not is_fc)
        and (not conv8.fit_weights)
        and conv8.mxp_double_buffer
        and (not conv8.use_depthwise)
    ):
        conv8.split_weight_shaper_buffers = SPLIT_WEIGHT_SHAPER_BUFFERS
    tile, allocation_succeed = allocation(
        node,
        state.preset,
        opcode,
        sparse=state.compression_vbx,
    ...
    if not allocation_succeed:
        return node, allocation_succeed

    conv8.first_fia = 0
    conv8.last_fia = 0
    conv8.fia_collision = 0
    if conv8.use_fia:
        if state.first_fia is False:
            conv8.first_fia = 1
            state.first_fia = True

        conv8.last_fia = 1

        if state.prev_fia_node:
            state.prev_fia_node.Conv2DOptions.last_fia = 0
            state.prev_fia_node.FullyConnectedOptions.last_fia = 0

        check_node_for_collision(node, state.Nodes, state.prev_fia_node)

        state.prev_fia_node = node
    ...
    if (
        conv8.use_fia
        and conv8.use_depthwise
        and conv8.dilation_height_factor != 1
        and h != 1
    ):
    ...
    if conv8.use_fia or USE_PRECALC:
        if opcode == "CONV_2D":
            bias_data += precalculate_filter_input_bias(
                filter_data, input_offset, reduce=False
            )
        else:
            bias_data += precalculate_filter_input_bias(
                filter_data, input_offset, reduce=True
            )
    # if PRECALC_OUTPUT: # 0
    ...
    if conv8.use_fia:
        _, output_multiplier, output_shift, c_input_L, c_input_H = (
            quantize_two_math_block(
                output_offset,
                node.activation_min,
                node.activation_max,
                op,
        ...
        with exception_catcher(node.type, len(state.Nodes), op["inputs"][0]):
            _, output_multiplier, output_shift = (
                get_effective_quantized_multiplier_from_op(op, tensors)
            )
    ...
    node.output_multiplier = len(state.weights)
    fmt = "{}i".format(len(output_multiplier))
    state.weights += struct.pack(fmt, *output_multiplier)

    node.output_shift = len(state.weights)
    fmt = "{}i".format(len(output_shift))
    state.weights += struct.pack(fmt, *output_shift)

    conv8.bias_data = len(state.weights)
    fmt = "{}i".format(len(bias_data))
    state.weights += struct.pack(fmt, *bias_data)

    _, _, _, _, _, rows, columns = tile
    weight_pad = 0

    # force byte alignment for weights
    if len(state.weights) % parallel_output_maps != 0:
        filler = parallel_output_maps - (len(state.weights) % parallel_output_maps)
        fmt = "{}x".format(filler)
        state.weights += struct.pack(fmt)
    conv8.filter_data = len(state.weights)
    conv8.repeat = -1
    filter_data = pad_weights_and_pack_2_1(
        filter_data,
        node,
    ...
    if node.tensor_array[0].type == calc_type.UINT8:
        fmt = "{}B".format(len(filter_data.flatten()))
    else:
        fmt = "{}b".format(len(filter_data.flatten()))
    state.weights += struct.pack(fmt, *filter_data.flatten())

    # pack quant records
    if len(state.weights) % QUANTIZATION_RECORD_WIDTH_BYTES != 0:
        filler = QUANTIZATION_RECORD_WIDTH_BYTES - (
            len(state.weights) % QUANTIZATION_RECORD_WIDTH_BYTES
        )
        fmt = "{}x".format(filler)
        state.weights += struct.pack(fmt)
    conv8.quantization_records = len(state.weights)

    if conv8.use_fia:
        for kernel in range(conv8.kernels):
            c_input_L_mask = (
                (c_input_L[kernel] >> 6) & ((2**C_INPUT_L_WIDTH) - 1)
            ) << C_INPUT_L_LSB
            c_input_H_mask = (
                c_input_H[kernel] & ((2**C_INPUT_H_WIDTH) - 1)
            ) << C_INPUT_H_LSB
            mult_mask = (
                output_multiplier[kernel] & ((2**MULTIPLIER_WIDTH) - 1)
            ) << MULTIPLIER_LSB
            shift_mask = (
                output_shift[kernel] & ((2**O_SHIFT_WIDTH) - 1)
            ) << O_SHIFT_LSB

            quant_record = shift_mask | mult_mask | c_input_H_mask | c_input_L_mask

            state.weights += struct.pack("<Q", quant_record)

    conv8.nlf_data = -1
    if state.nlf_data is not None:
        conv8.nlf_data = len(state.weights)
        fmt = "{}b".format(len(state.nlf_data))
        state.weights += struct.pack(fmt, *state.nlf_data.flatten())
        op["outputs"] = state.lut_ops[-1][0]["outputs"]
    elif 0:  # force passthru NLF
        state.nlf_data = np.asarray([_ for _ in range(0, 256)], dtype=np.uint8).astype(
            np.int8
        )
        conv8.nlf_data = len(state.weights)
        fmt = "{}b".format(len(state.nlf_data))
        state.weights += struct.pack(fmt, *state.nlf_data.flatten())
    # nlf pack unchanged (omitted for brevity if not present)
    return node, allocation_succeed


def _process_batch_matmul_node(g, node, op, opts, tensors, opcode, buffers, state):
    for t in op["inputs"][1:]:
        ...
        tn.name = tensor["name"]
        if not node.offloaded and state.engine_graphs_nx:
            add_external_producer_consumer_info(
                tn,
                state.external_inputs,
                state.external_outputs,
                state.already_external_producer,
                state.already_external_consumer,
                g,
            )
        node.tensor_array.append(tn)

    node.m, node.n = max(node.m, shape[-2]), max(
        node.n, shape[-1]
    )  # increase size (to handle channelwise broadcast)
...
def _process_fully_connected_node(g, node, op, opts, tensors, opcode, buffers, state):

    input_offset, _, output_offset, _, i_tensor, _ = (
        get_quantization_params_from_tensor(node, tensors, op, buffers)
    )
    input_shape, idims = channels_first_shape(i_tensor["shape"])
    t_shape = input_shape
    if idims >= 3:
        node.channels = t_shape[-2]
        node.m = t_shape[-1]
        node.n = t_shape[-3]
    elif idims >= 2:
        node.channels = 1
    ...
    if state.engine_graphs_nx is not None:
        fc8.use_fia = 0
    else:
    ...
    fc8.bias_data = len(state.weights)
    fmt = "{}i".format(len(bias_data))
    state.weights += struct.pack(fmt, *bias_data)

    fc8.filter_data = len(state.weights)
    if fc8.use_fia:
        filter_data = pad_weights_and_pack_2_1(
            filter_data,
            node,
            preset_select["PARALLEL_OUTPUT_MAPS"][state.preset],
            tile,
            state.preset,
            opcode,
            sparse=state.compression_vbx,
    ...
    if len(state.weights) % QUANTIZATION_RECORD_WIDTH_BYTES != 0:
        ...
        fmt = "{}x".format(filler)
        state.weights += struct.pack(fmt)
    fc8.quantization_records = len(state.weights)

    if fc8.use_fia:
        for out in range(output_depth):
            c_input_L_mask = (
                (c_input_L[out] >> 6) & ((2**C_INPUT_L_WIDTH) - 1)
            ) << C_INPUT_L_LSB
            c_input_H_mask = (
                c_input_H[out] & ((2**C_INPUT_H_WIDTH) - 1)
            ) << C_INPUT_H_LSB
            mult_mask = (
                output_multiplier[0] & ((2**MULTIPLIER_WIDTH) - 1)
            ) << MULTIPLIER_LSB
    ...
    fc8.nlf_data = -1
    if state.nlf_data is not None:
        fc8.nlf_data = len(state.weights)
        fmt = "{}b".format(len(state.nlf_data))
        state.weights += struct.pack(fmt, *state.nlf_data.flatten())
        op["outputs"] = state.lut_ops[-1][0]["outputs"]
...
def _process_transpose_conv_node(g, node, op, opts, tensors, opcode, buffers, state, json_graph):
    ...
    pad_h, pad_w = 0, 0
    if opts.get("padding", None) == "VALID":
        conv8.padding_width = 0
        conv8.padding_height = 0
    ...
    elif opts.get("padding", None) == "SAME":
    ...
    # identity+pad or just pad should only be injected if
    # strides are 2,2 and so the input must be dilated with a modified Pad
    inject_identity = False
    ...
    if conv8.use_fia:
        ...
        _, output_multiplier, output_shift = (
            get_effective_quantized_multiplier_from_tensors(
                i_tensor, o_tensor, f_tensor
            )
        )
...
def _process_identity_node(
    g, node, op, opts, tensors, opcode, buffers, multi_input, state
):
    ...
    if multi_input:
        ...
        if len(shape) > 3:
            node.channels = max(node.channels, shape[-3])
...
def _process_concat_node(g, node, op, opts, tensors, opcode, buffers, state):

    _, _, _, _, _, o_tensor = get_quantization_params_from_tensor(
        node, tensors, op, buffers
    )  # to set input and output offsets
...
def _process_pack_node(g, node, op, opts, tensors, opcode, buffers, state):

    _, _, _, _, i_tensor, o_tensor = get_quantization_params_from_tensor(
        node, tensors, op, buffers
    )
    ...
    assert check_pack(
        ...
        o_tensor["shape"],
        opts["axis"],
        node.PackOptions.axis,
        node.PackOptions.count,
    )
...
def _process_unpack_node(g, node, op, opts, tensors, opcode, buffers, state):

    _, _, _, _, i_tensor, o_tensor = get_quantization_params_from_tensor(
        node, tensors, op, buffers
    )
    nput_shape, idims = channels_first_shape(i_tensor["shape"])
...
def _process_split_splitV_node(g, node, op, opts, tensors, opcode, buffers, state):
    ...
    node.SplitOptions.splits = len(state.weights)
    node.SplitOptions.max_split = max(splits)

    fmt = "{}i".format(len(splits))
    state.weights += struct.pack(fmt, *splits)
...
def _process_tile_node(g, node, op, opts, tensors, opcode, buffers, state):
    _, _, _, _, i_tensor, _ = get_quantization_params_from_tensor(
        node, tensors, op, buffers
    )  # to set input and output offsets
    idims = len(i_tensor["shape"])
...
def _process_mean_node(g, node, op, opts, tensors, opcode, buffers, state):

    _, _, _, _, i_tensor, o_tensor = get_quantization_params_from_tensor(node, tensors, op, buffers)
    idims = len(i_tensor['shape'])
    odims = len(o_tensor['shape'])
    input_shape, idims = channels_first_shape(i_tensor['shape'])
    input_offset = i_tensor['quantization']['zero_point'][0]
    ...
    axis = get_numpy_data_from_index(op['inputs'][1], tensors, buffers)
    if axis.tolist() == [1,2,3]:
        node.reduce8.channels, node.reduce8.rows, node.reduce8.columns = 1, 1, 1
        num_elements_in_axis = input_shape[-3]*input_shape[-2]*input_shape[-1]
    elif axis.tolist() == [1,2]:
        node.reduce8.channels, node.reduce8.rows, node.reduce8.columns = 0, 1, 1
        num_elements_in_axis = input_shape[-2]*input_shape[-1]
    else:
        print('currently not supporting other reduction axis combinations')

    _, output_multiplier, output_shift = get_effective_quantized_multiplier_from_op(op, tensors, [1.0/num_elements_in_axis,])
    ...
    tile, allocation_succeed = allocation(node, state.preset, opcode, sparse=state.compression_vbx, tmp_dir=state.tmp_dir, graph_idx=g, tmp_dir_obj=state.tmp_dir_obj, json_graph=state.json_graph, split=state.splits[g])

    return node, allocation_succeed


def op_split_groups(ops, codes, tensors, buffers):

    # force ops to stay together
    extra_patterns = [
    ]

    # argmax pattern is to keep together since argmax output is currently only supported as an external output 
    # if it is the output of a standalone op, and if it is consumed by another op then the consumer op needs to be able to find the argmax output tensor 
    # in the graph which is easier if they are grouped together. The reshape in the pattern is for a specific case where there is a reshape after argmax+cast
    #  that needs to be grouped with them for the consumer op to find the argmax output tensor, but we may want to expand this pattern in the future if 
    # there are other similar cases where there is a reshape after argmax+cast that causes issues with finding the argmax output tensor in the graph.
    #look LUT table funchtions together since they need to be grouped together for the consumer op to find the LUT output tensor in the graph, and also 
    # if there are multiple LUT ops in a row then they need to be grouped together for the consumer op to find the correct LUT output tensor in the graph

    groups = []
    i = 0
    while i < len(ops):
        _, pattern = graph_pattern(ops[i:], codes, tensors, buffers, extra_patterns)
...
def _process_node_and_subnodes(
    g,
    codes,
    layer_codes,
    subcodes,
    ...
    opcode = [str(codes[_["opcode_index"]]) for _ in ops]
    opcode = opcode[0]
    state.lut_ops = None
    state.reshape_ops = None
    state.nlf_data = None
    op = ops[0]
    opts = None
    i = 0
    while i < len(subops):
        ...
        if len(pattern):
            ...
            if subcode == "LUT":
                state.lut_ops = list(zip(subops_, subcodes_, pattern))
                if (
                    node.type
                    in [
                        BuiltinOperator.CONV_2D,
                    ...
                    and len(pattern) == 3
                    and CONV_NLF
                    and tensors[subops_[2]["outputs"][0]]["type"] in ["INT8"]
                ):  # TODO add UINT8 support
                    state.nlf_data = get_numpy_data_from_index(
                        subops_[2]["inputs"][0], tensors, buffers
                    )
                    state.nlf_data = lut_i8_to_u8(state.nlf_data)
                    i += len(pattern)
                    continue
            elif subcode == "TRANSFORM":
                state.reshape_ops = list(zip(subops_, subcodes_, pattern))
        else:
            subops_ = [subops[i]]
            subcode = subcodes[i]
        layer_codes.append(subcode)
        subnode_array, subnode_tensors, prev_subop = populate_subnodes(
            subcode,
            ops,
            state.lut_ops,
            state.reshape_ops,
            ...
            buffers,
            state.weights,
            state.dequantize_op_params,
            state.aliased_ids,
            state.tmp_dir,
            state.engine_graphs_nx,
            state.external_inputs,
            state.external_outputs,
            state.already_external_producer,
            state.already_external_consumer,
    ...
    if node.type == BuiltinOperator.CONV_2D:
        ...
        node, allocation_succeed = _process_conv2d_node(
            g, node, op, opts, tensors, opcode, buffers, state, json_graph
        )
        ...
        node, allocation_succeed = _process_transpose_conv_node(
            g, node, op, opts, tensors, opcode, buffers, state, json_graph
        )

    elif node.type == BuiltinOperator.FULLY_CONNECTED:

        node, allocation_succeed = _process_fully_connected_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == BuiltinOperator.BATCH_MATMUL:  # TODO not adding explicit tensor

        node, allocation_succeed = _process_batch_matmul_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == VNNXOperator.IDENTITY:  # TODO not adding explicit tensor

        node, allocation_succeed = _process_identity_node(
            g, node, op, opts, tensors, opcode, buffers, multi_input, state
        )

    elif node.type == BuiltinOperator.CONCATENATION:

        node, allocation_succeed = _process_concat_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == BuiltinOperator.PACK:

        node, allocation_succeed = _process_pack_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == BuiltinOperator.UNPACK:

        node, allocation_succeed = _process_unpack_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )
        ...
        node, allocation_succeed = _process_split_splitV_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == BuiltinOperator.TILE:

        node, allocation_succeed = _process_tile_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )

    elif node.type == BuiltinOperator.MEAN:

        node, allocation_succeed = _process_mean_node(
            g, node, op, opts, tensors, opcode, buffers, state
        )
    ...
    return node, allocation_succeed, subnode_array, subnode_tensor_array
...
def populate_nodes(
    ...
    state = NodeProcessingState(
        splits=splits,
        preset=preset,
        weights=weights,
        aliased_ids=aliased_ids,
        ids_with_dummies=ids_with_dummies,
        in_id_to_out_ids=in_id_to_out_ids,
        tmp_dir=tmp_dir,
        tmp_dir_obj=tmp_dir_obj,
        compression_vbx=compression_vbx,
        engine_graphs_nx=engine_graphs_nx,
    ...
    valid_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "supported_ops.json"
    )
    valid = json_load(valid_path)

    def is_vectorized(x):
        return x in valid and valid[x]["vectorized"][0] >= 1
    ...
    while g < len(state.splits):
        ...
        if state.engine_graphs_nx:
            # Set whether or not this node is going to be offloaded to NX
            node.offloaded = is_split_idx_in_engine_graphs(g, state.engine_graphs_nx)
        ...
        for sig_input in sig["inputs"]:
            input_type = calc_type.from_str(tensors[sig_input["tensor_index"]]["type"])
            break

        for sig_output in sig["outputs"]:
            output_type = calc_type.from_str(
                tensors[sig_output["tensor_index"]]["type"]
            )
            break
        ...
        multi_input = len(input_buffers) > 1 and not any(
            ["data" in _ for _ in input_buffers]
        )
        ...
        DATA_MOVEMENT += ["UNPACK", "RESHAPE", "EXPAND_DIMS", "SQUEEZE"]
        DATA_MOVEMENT += [
            "DEPTH_TO_SPACE",
            "SPACE_TO_DEPTH",
            "BATCH_TO_SPACE_ND",
        ...
        BINARY = ["ADD", "SUB", "MUL", "DIV", "SQUARED_DIFFERENCE"]
        TYPE_CAST = ["CAST"]
        ...
        if opcode in ["CONV_2D", "DEPTHWISE_CONV_2D"]:
            ...
            subops = ops[1:]
            subcodes = opcodes[1:]
        elif opcode in ["CONCATENATION", "PACK", "SPLIT", "SPLIT_V", "TILE", "MEAN"]:
            ...
            opcode in ["QUANTIZE", "DEQUANTIZE"]
            or opcode in VALID_SUBNODE
            or opcode in POST_PROCESSING
        ):
            node.type = VNNXOperator.IDENTITY
            subops = ops
            subcodes = opcodes
        ...
        # Process node and attempt allocation
        node, allocation_succeed, subnode_array, subnode_tensor_array = (
            _process_node_and_subnodes(
                g,
                codes,
                layer_codes,
                subcodes,
        ...
        if not allocation_succeed:
            split_groups = op_split_groups(ops, codes, tensors, buffers)
            if len(split_groups) > 1:
                # Allocation failed but can split - modify splits and restart this iteration
                # Using continue restarts the outer loop with same g, but state.splits[g] now points to first part
                # This ensures fresh graph/ops/opcodes/subops/subcodes are computed from the new split
                # print('Allocation failed for graph {}, splitting...'.format(state.splits[g]))

                cut_idx = split_groups[-1][0]  # cut last subops?

                cut_idx = split_groups[1][0]  # cut after first ops?
                if state.nlf_data is not None and len(split_groups) > 2:
                    cut_idx = split_groups[2][0]  # cut after first ops?

                new_splits = [state.splits[g][:cut_idx]] + [state.splits[g][cut_idx:]]
                state = prev_state

                state.splits = state.splits[:g] + new_splits + state.splits[g + 1 :]

                # Recompute in_id_to_out_ids with updated splits
                # This is needed because tensor relationships change after split
                state.in_id_to_out_ids = tensor_in_id_to_out_id_mapping(
                    json_graph, state.splits
                )
                continue  # Restart outer loop - will recompute graph, ops, subops from new split
            else:
                # Cannot split further - raise error
                with sp_invalid_exception_catcher(
                    preset_select["SCRATCHPAD_KB"][preset] * 1024 - 256,
                    opcode,
                    json_graph,
                    state.splits[g],
                    g,
        ...
        node.num_outputs = len(ops[0]["outputs"])
        for t in ops[0]["outputs"]:
            tensor = tensors[t]
            ...
            if node.type == BuiltinOperator.CONV_2D and node.Conv2DOptions.use_fia:
                if (
                    node.Conv2DOptions.use_depthwise
                    and node.Conv2DOptions.stride_width != 1
                ):
                    inject_strided_slice(
                        node, tn, subnode_tensor_array, state.ids_with_dummies
                    )
                    ...
                    tile, _ = allocation(
                        ...
                        graph_idx=g,
                        tmp_dir_obj=state.tmp_dir_obj,
                    )  # re-allocate as stride_width forced to 1
                    assert not (tile is None)

                oh, ow, osh, osw = get_output_shapes(
                    node.Conv2DOptions, node.rows_0, node.cols_0
                )
                if (oh * ow != osh * osw) and (not node.offloaded):
                    inject_output_shaper(
                        node, tn, subnode_tensor_array, state.ids_with_dummies
                    )
        ...
        if (
            node.type == VNNXOperator.IDENTITY and node.num_inputs == 1
        ):  # injected identity, rename tensors TODO fix for multi-input
        ...
        state.Nodes_to_cuts_dict[node] = state.splits[g]
        g += 1

    state.Nodes = update_nodes_with_dequant_params(
        state.Nodes, state.dequantize_op_params
    )
    return state.Nodes
...
def check_inputs_outputs_quantization(first_input, first_output):
    # check if the input and the output have the equally quantization values, and if multi-input then check that all inputs have same quantization values

    input_scale = first_input.get("quantization", {}).get("scale", [1.0])[0]
    input_zero = first_input.get("quantization", {}).get("zero_point", [0])[0]
    output_scale = first_output.get("quantization", {}).get("scale", [1.0])[0]
    output_zero = first_output.get("quantization", {}).get("zero_point", [0])[0]

    # Check if input and output have equal quantization
    input_output_match = (input_scale == output_scale) and (input_zero == output_zero)

    return input_output_match


def populate_subnodes(
    subcode,
    ops,
    lut_ops,
    reshape_ops,
    ...
    i_tensor = tensors[subop["inputs"][0]]
    o_tensor = tensors[subops[-1]["outputs"][0]]
    ...
    if subcode not in [
        ...
        assert (
            sn.input_data_type == calc_type.INT8
            or sn.input_data_type == calc_type.UINT8
        )
    ...
    if subcode == "DEPTHWISE_CONV_2D":
        ...
        f_tensor = tensors[subop["inputs"][1]]
        b_tensor = None
        # TODO `op` does not exist in this function and this would never be true, possibly deprecated
        # if len(op["inputs"]) > 2 and op["inputs"][2] != -1:
        #    b_tensor = tensors[op["inputs"][2]]
        ...
        if k == 1 and c > 1 and subcode == "DEPTHWISE_CONV_2D":
            ...
            assert opts.get("padding", None) == "VALID"
            ...
            # effective_scale unused (would be used in PRECALC_OUTPUT)
            # TODO this might be deprecated due to prev iteration using node being undefined
            if conv8.use_fia:
                _, output_multiplier, output_shift, c_input_L, c_input_H = (
                    quantize_two_math_block(
                        output_offset,
                        sn.activation_min,  # node.activation_min,
                        sn.activation_max,  # node.activation_max,
                        subop,
                        tensors,
                        bias_data=bias_data,
                    )
                ...
                _, output_multiplier, output_shift = (
                    get_effective_quantized_multiplier_from_op(subop, tensors)
                )
        ...
        and multi_input
    ):  # >>>>>>>>> Main ELTWISE BLOCK with multi_inputs
        ...
        if (
            subcode == "ADD"
            and (2 * input_scale[0] / output_scale[0]) < 2 ** (15 - Q16)
            and (2 * input2_scale[0] / output_scale[0]) < 2 ** (15 - Q16)
        ):
            if len(i_tensor["shape"]) > 3:  # TODO fix post-procesing accuracy drop
                eltwise8.optimized = OPTIMIZED_ADD
            if check_inputs_outputs_quantization(
                i_tensor, o_tensor
            ) and check_inputs_outputs_quantization(i2_tensor, o_tensor):
                # set optimized_add =2 when scaling factors are equal to enable more aggressive optimization in c code (e.g. skipping scaling of all inputs)
                eltwise8.optimized = 2
            elif (
                check_inputs_outputs_quantization(i_tensor, o_tensor)
                and not check_inputs_outputs_quantization(i2_tensor, o_tensor)
                and input_offset != input2_offset
            ):
                # set optimized_add =3 quntization values of input1 match output to enable optimization in c code (e.g. skipping scaling of input1)
                eltwise8.optimized = 3
            elif (
                not check_inputs_outputs_quantization(i_tensor, o_tensor)
                and check_inputs_outputs_quantization(i2_tensor, o_tensor)
                and input_offset != input2_offset
            ):
                # set optimized_add =4 quntization values of input2 match output to enable optimization in c code (e.g. skipping scaling of input2)
                eltwise8.optimized = 4
            ...
            if OPTIMIZED_ADD and VERBOSE:
                print(
                    ...
                    input2_scale[0] / output_scale[0],
                    "WARNING can't optimize multi",
                )
        ...
        if subcode in ["ADD", "SUB"]:
            ...
            real_input_multiplier = (
                np.asarray(input_scale) / twice_max_input_scale
            ).tolist()
            real_input2_multiplier = (
                np.asarray(input2_scale) / twice_max_input_scale
            ).tolist()
            real_output_multiplier = (
                twice_max_input_scale / (2**left_shift * np.asarray(output_scale))
            ).tolist()

            input_multiplier, input_shift = get_quantized_multiplier(
                real_input_multiplier
            )
            input2_multiplier, input2_shift = get_quantized_multiplier(
                real_input2_multiplier
            )
            output_multiplier, output_shift = get_quantized_multiplier(
                real_output_multiplier
            )
            ...
            real_output_multiplier = (
                ...
                / ((1 << left_shift * 2) * np.asarray(output_scale))
            ).tolist()
            ...
            if input1_shape != input2_shape:
                if len(input2_shape) > 1:
                    sys.stderr.write(
                        "ERROR: SUBOP{} doesn't support difference tensor shapes: {}, {}.\n".format(
                            subcode, input1_shape, len(input2_shape)
                        )
                    )
                    sys.exit(1)
        ...
        while l < len(reshape_ops):
            ...
            if (
                code == "RESHAPE"
                and next_code == "TRANSPOSE"
                and next_next_code == "RESHAPE"
            ):
                l += 2
                bytes = 1
                ...
                o_tensor = tensors[next_next_op["outputs"][0]]
                first_o_tensor = tensors[op["outputs"][0]]
                output_shape, odims = channels_first_shape(o_tensor["shape"])
                output_type = o_tensor["type"]
                sn.output_data_type = calc_type.from_str(output_type)
                # ARG_MAX outputs indices, no quantization offset needed
                final_output_offset = 0
                final_output_scale = 1.0

                sn.type = BuiltinOperator.ARG_MAX
                input_offset = 0
                output_offset = 0

                input_scale = 1.0
                output_scale = 1.0
                assert len(first_o_tensor["shape"]) == len(i_tensor["shape"]) - 1

                sn.reduce8.channels, sn.reduce8.rows, sn.reduce8.columns = 0, 0, 0
                axis = get_numpy_data_from_index(op["inputs"][1], tensors, buffers)

                if axis == -1 or axis == 3:
                    sn.reduce8.channels = 1
                ...
                code == "ARG_MAX"
                and next_code == "RESHAPE"
                and next_next_code == "CAST"
            ):
                # ARG_MAX -> RESHAPE -> CAST pattern (3 ops)
                l += 2
                ...
                # ARG_MAX -> RESHAPE pattern (2 ops, must come after 3-op patterns)
                l += 1
        ...
        # Only set multiplier/shift for PIXEL_SHUFFLE, not for ARG_MAX
        if sn.type == VNNXOperator.PIXEL_SHUFFLE:
            sn.input_shift = -1
        ...
        concat_ops = [_ for _ in lut_ops if _[1] == "CONCATENATION"]
        gather_ops = [_ for _ in lut_ops if _[1] == "GATHER"]

        if len(concat_ops):
            concat_op = concat_ops[0][0]
            gather_indices = []
            for input_idx in concat_op["inputs"]:
                for g, (gop, _, _) in enumerate(gather_ops):
                    if input_idx in gop["outputs"]:
                        gather_indices.append(g)
                        break
            gather_ops = [gather_ops[i] for i in gather_indices]

        for lop, code, _ in gather_ops:
            l = get_numpy_data_from_index(lop["inputs"][0], tensors, buffers)
            if sn.input_data_type in [
                calc_type.INT8,
                calc_type.INT32,
            ]:
                l = lut_i8_to_u8(l)
        ...
        resize.scale = (
            new_size_data / i_tensor["shape"][1:3]
        ).tolist()  # height and width scales

        c_num_copies = []
        steps = [0]  # Only needed for bilinear --> so empty for NN
        if all([sc > 1 for sc in resize.scale]):
            c_num_copies, c_out = resize_nearest_indices(
                i_tensor["shape"][1:3], new_size_data
            )

        sn.ResizeOptions.num_c_inc = len(c_num_copies)
        sn.ResizeOptions.min_rows = 0
        sn.ResizeOptions.min_cols = 0
        sn.ResizeOptions.min_maps = 0
        sn.ResizeOptions.rows_trips = 1
        sn.ResizeOptions.cols_trips = 1
        sn.ResizeOptions.maps_trips = 1
        sn.ResizeOptions.in_scratchpad = 0
        ...
        resize.ratio = (
            (1 << 10) * i_tensor["shape"][1] + o_tensor["shape"][1] / 2
        ) / o_tensor["shape"][1]
        ...
        c_num_copies, steps = [], []
        c_num_copies, c_out, steps = resize_bilinear_indices(
            i_tensor["shape"][1:3], new_size_data
        )
        # print(c_num_copies)
        sn.ResizeOptions.num_c_inc = len(c_num_copies)
        ...
        sn.ResizeOptions.cols_trips = 1
        sn.ResizeOptions.maps_trips = 1
        sn.ResizeOptions.in_scratchpad = 1
        ...
        output_scale = o_tensor["quantization"]["scale"]
        output_multiplier = 1.0 / (output_scale[0] * 2**16)
        ...
        # transform = lambda s: np.exp((s - 127) * input_scale[0]) * lut_scale #TODO remove this once verified
        def transform(s):
            transformed_s = np.exp((s - 127) * input_scale[0]) * lut_scale
            return transformed_s

        lut, first, last, step_vals, step_indices = LUTPopulate(
            1.0, 0.0, 1.0, 0, transform, bytes=4
        )
        ...
        input_beta_real_multiplier = min(
            kBeta * input_scale[0] * (1 << (31 - input_integer_bits)),
            max_real_multiplier,
        )
        assert input_beta_real_multiplier > 1.0
        quantized_multiplier, left_shift = get_quantized_multiplier(
            [input_beta_real_multiplier]
        )
        assert all(x >= 0 for x in left_shift)
        assert i_tensor["shape"] == o_tensor["shape"]

        real_reverse_scaling_divisor = (
            1 << (31 - left_shift[0])
        ) / quantized_multiplier[0]
        assert real_reverse_scaling_divisor < 1.0 and real_reverse_scaling_divisor > 0.0
        reverse_scaling_divisor, reverse_scaling_left_shift = get_quantized_multiplier(
            [real_reverse_scaling_divisor]
        )
        ...
        effective_scale, output_multiplier, output_shift = (
            get_effective_quantized_multiplier_from_op(subop, tensors)
        )
        ...
        if (
            broad8.filter_shape_dims[-2] == 1 and broad8.filter_shape_dims[-1] == 1
        ):  # channels
            broad8.broadcast = 1
        elif (
            broad8.filter_shape_dims[-2] == 1
            and broad8.filter_shape_dims[-1] == ishape[-1]
        ):  # row
            broad8.broadcast = 2
        elif (
            broad8.filter_shape_dims[-2] == ishape[-2]
            and broad8.filter_shape_dims[-1] == ishape[-1]
        ):  # maps
            broad8.broadcast = 3
        ...
        real_filter_multiplier = (
            np.asarray(filter_scale) / twice_max_input_scale
        ).tolist()
        real_output_multiplier = (
            twice_max_input_scale / (2**left_shift * np.asarray(output_scale))
        ).tolist()
        ...
        filter_multiplier, filter_shift = get_quantized_multiplier(
            real_filter_multiplier
        )
        output_multiplier, output_shift = get_quantized_multiplier(
            real_output_multiplier
        ...
        if (
            broad8.broadcast == 1
            and (2 * input_scale[0] / output_scale[0]) < 2 ** (15 - Q16)
            and (2 * filter_scale[0] / output_scale[0]) < 2 ** (15 - Q16)
        ):
        ...
        if axis.tolist() == [1, 2, 3]:
            sn.reduce8.channels, sn.reduce8.rows, sn.reduce8.columns = 1, 1, 1
            num_elements_in_axis = input_shape[-3] * input_shape[-2] * input_shape[-1]
        elif axis.tolist() == [1, 2]:
            sn.reduce8.channels, sn.reduce8.rows, sn.reduce8.columns = 0, 1, 1
            num_elements_in_axis = input_shape[-2] * input_shape[-1]
        else:
            print("currently not supporting other reduction axis combinations")

        if subcode == "REDUCE_PROD":
            _, output_multiplier, output_shift = get_quantized_multiplier_from_tensor(
                o_tensor
            )
        elif subcode == "MEAN":
            _, output_multiplier, output_shift = (
                get_effective_quantized_multiplier_from_op(
                    subop,
                    tensors,
                    [
                        1.0 / num_elements_in_axis,
                    ],
                )
            )
        else:
        ...
        transform_size = len(
            np.trim_zeros([_ if i == _ else 0 for i, _ in enumerate(transform)], "f")
        )
        ...
        transform = tuple(
            get_numpy_data_from_index(subop["inputs"][1], tensors, buffers)
        )
        ...
        sn.nop = 0
        if mode in [0, 4, 8] and len(ishape) <= len(oshape):
            sn.nop = 1
        ...
        if len(dequantize_op_params) % 2 != 0:
            ...
            real_output_multiplier = (
                np.asarray(input_scale) / np.asarray(output_scale)
            ).tolist()
            ...
            mixed_type_int8_uint8 = (
                i_type == calc_type.INT8 and o_type == calc_type.UINT8
            )
            mixed_type_uint8_int8 = (
                i_type == calc_type.UINT8 and o_type == calc_type.INT8
            )
        ...
        if len(alpha_tensor["shape"]) == 3:
            ...
            sys.stderr.write(
                "ERROR: broadcast on 2D or 3D input is not supported yet in {}\n".format(
                    subcode
                )
            )
            sys.exit(1)
        ...
        real_alpha_multiplier = (
            np.asarray(input_scale) * np.asarray(alpha_scale) / np.asarray(output_scale)
        ).tolist()
        ...
        if subcode == "RELU6":
            sn.activation_max = min(
                127, output_offset + int(round(6.0 / output_scale[0]))
            )
        elif subcode == "RELU_0_TO_1":
            # sn.activation_min = max(-128, output_offset + int(round(0.0 / output_scale[0])))
            sn.activation_max = min(
                127, output_offset + int(round(1.0 / output_scale[0]))
            )
        elif subcode == "RELU_N1_TO_1":
            sn.activation_min = max(
                -128, output_offset + int(round(-1.0 / output_scale[0]))
            )
            sn.activation_max = min(
                127, output_offset + int(round(1.0 / output_scale[0]))
            )
        ...
        if axis == -1 or axis == 3:
            sn.reduce8.channels = 1  # vnnx channels first
            axis_list = np.array(range(input_shape[1]))
            sn.reduce8.axis_list = len(weights)
            fmt = "{}b".format(len(axis_list.flatten()))
            weights += struct.pack(fmt, *axis_list.flatten())
            ...
            sys.stderr.write(
                "ERROR: reduction on axis {} is not supported in {}\n".format(
                    axis, subcode
                )
            )
            sys.exit(1)
        elif axis == -2 or axis == 2:
            sn.reduce8.rows = 1  # vnnx channels first
            sys.stderr.write(
                "ERROR: reduction on axis {} is not supported in {}\n".format(
                    axis, subcode
                )
            ...
            sn.reduce8.columns = 1  # vnnx channels first
            sys.stderr.write(
                "ERROR: reduction on axis {} is not supported in {}\n".format(
                    axis, subcode
                )
        ...
        sn.SliceOptions.end = [
            min(_, ishape[idx]) if _ != 0 else ishape[idx] for idx, _ in enumerate(end)
        ]
        ...
        if batch_dims != 0:
            sys.stderr.write(
                "ERROR: batch_dim = {} for GATHER is not implemented\n".format(
                    batch_dims
                )
            )
        ...
        block_shape_data = get_numpy_data_from_index(
            subop["inputs"][1], tensors, buffers
        )
