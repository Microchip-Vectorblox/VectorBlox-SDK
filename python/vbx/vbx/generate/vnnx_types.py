...
preset_select = {
    "VECTOR_LANES": {"V250": 2, "V500": 4, "V1000": 8, "V2000": 16, "V4000": 16},
    "FILTER_COPIES": {"V250": 8, "V500": 8, "V1000": 16, "V2000": 32, "V4000": 32},
    ...
    "SCRATCHPAD_KB": {"V250": 32, "V500": 64, "V1000": 128, "V2000": 256, "V4000": 256},
    "PRESET": {"V250": 0, "V500": 1, "V1000": 2, "V2000": 3, "V4000": 4},
}
...
Subnode_struct = [
    ...
    (
        "union",
        {
            ...
            "leakyrelu": [("offset", "alpha_multiplier"), ("offset", "alpha_shift")],
            "SoftmaxOptions": [
                ("int32_t", "diff_min"),
                ("int32_t", "axis"),
                ("int32_t", "depth"),
...
class Struct:
    ...
    def print_structure(self):
        print(
            "{} ({} bytes)".format(self.__class__.__name__, self.get_structure_size())
        )
        fmt = []
        union_fmt = {"": []}
        ...
        # get_format = lambda l: "<" + "".join([fmt for nm, fmt in l]) #TODO remove this
        def get_format(fields):
            return "<" + "".join(fmt for _, fmt in fields)
...
class TileLoadStep:
    """
    Sequential load plan entry with explicit row/col/map fields so it maps
    directly to node tile-step attributes (row_start/row_inc/... etc).
    Only the fields relevant to a given step will be populated.
    """
    ...
    # input-maps (imaps) fields
    imap_start: int = 0
    imap_length: int = 0
    imap_inc: int = 0
    imap_total: int = 0

    # output-maps (maps) fields
    map_start: int = 0
    map_length: int = 0
    map_inc: int = 0
    map_total: int = 0
...
def enum_to_union_name(e):
    union_names = [
        (BuiltinOperator.CONV_2D, "Conv2DOptions"),
        ...
        (BuiltinOperator.TOPK_V2, "reduce8"),
        (BuiltinOperator.REDUCE_MAX, "reduce8"),
        (BuiltinOperator.REDUCE_MIN, "reduce8"),
        (BuiltinOperator.REDUCE_PROD, "reduce8"),
        (BuiltinOperator.SUM, "reduce8"),
        (BuiltinOperator.MEAN, "reduce8"),
    ...
    for t, n in union_names:
        if isinstance(t, type(e)) and t == e:
            return n
    return ""
...
def graph_version(script_dir=None):
    ...
    if not os.path.exists(vnnx_types_path):
        vnnx_types_path = os.path.join(
            script_path, "../../../../libvnnx/include/vnnx-types.h"
        )
