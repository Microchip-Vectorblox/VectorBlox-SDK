...
echo "Checking for yolov2-tiny-voc files..."
...
if [ ! -f yolov2-tiny-voc.json ]; then
...
if [ -f yolov2-tiny-voc.cfg ]; then
   if ! echo "9cfd76e79908ec28aab1becfefa1a032 yolov2-tiny-voc.cfg" | md5sum -c; then
       echo -e "\n There is an issue with the yolov2-tiny-voc model file as the expected checksum does not match.\n The model source can be found at: https://pjreddie.com/darknet/yolo/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov2-tiny-voc.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov2-tiny-voc.pre.tflite  -o yolov2-tiny-voc_V1000_ncomp.vnnx
fi
...
if [ -f yolov2-tiny-voc_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov2-tiny-voc_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -j yolov2-tiny-voc.json -v 2 -l voc.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov2-tiny-voc_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg YOLOV2'
fi
