...
echo "Downloading FFNet-122NS-LowRes_512x288..."
# model details @ 
[ -f FFNet-122NS-LowRes.onnx ] || wget -q --no-check-certificate https://huggingface.co/qualcomm/FFNet-122NS-LowRes/resolve/main/FFNet-122NS-LowRes.onnx
python - <<EOF
import onnx
...
if [ -f FFNet-122NS-LowRes_512x288.pre.tflite ]; then
   tflite_postprocess FFNet-122NS-LowRes_512x288.pre.tflite  --dataset cityscapes \
--opacity 0.8 \
--height 1080 \
--width 1920
fi
...
if [ -f FFNet-122NS-LowRes_512x288.pre.post.tflite ]; then
    ...
    vnnx_compile -c V1000 -t FFNet-122NS-LowRes_512x288.pre.post.tflite -o FFNet-122NS-LowRes_512x288.vnnx
fi

if [ -f FFNet-122NS-LowRes_512x288.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/segmentation.py FFNet-122NS-LowRes_512x288.vnnx $VBX_SDK/tutorials/test_images/A0PQ76.jpg --dataset cityscapes --inj 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model FFNet-122NS-LowRes_512x288.vnnx $VBX_SDK/tutorials/test_images/A0PQ76.jpg  '
fi
