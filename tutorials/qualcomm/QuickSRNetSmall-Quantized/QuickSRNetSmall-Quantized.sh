...
if [ -f QuickSRNetSmall-Quantized.pre.tflite ]; then
    ...
    vnnx_compile -c V1000 -t QuickSRNetSmall-Quantized.pre.tflite -o QuickSRNetSmall-Quantized.vnnx
fi

if [ -f QuickSRNetSmall-Quantized.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/super_resolution.py QuickSRNetSmall-Quantized.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model QuickSRNetSmall-Quantized.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg  '
fi
