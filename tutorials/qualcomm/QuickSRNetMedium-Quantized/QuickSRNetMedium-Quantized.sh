...
if [ -f QuickSRNetMedium-Quantized.pre.tflite ]; then
    ...
    vnnx_compile -c V1000 -t QuickSRNetMedium-Quantized.pre.tflite -o QuickSRNetMedium-Quantized.vnnx
fi

if [ -f QuickSRNetMedium-Quantized.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/super_resolution.py QuickSRNetMedium-Quantized.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model QuickSRNetMedium-Quantized.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg  '
fi
