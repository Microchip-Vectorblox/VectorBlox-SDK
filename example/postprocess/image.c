...
int read_JPEG_file(char * filename,int* width,int*height,unsigned char** image, const int grayscale)
{
  ...
  *image = malloc(row_stride*cinfo.output_height);
  while (cinfo.output_scanline < cinfo.output_height) {
	...
	memcpy(*image+row_stride*(cinfo.output_scanline-1),buffer[0],row_stride);
...
/**
 * bilinear interpolation with planer input and planer output
 */
#include <stdint.h>
void resize_image(uint8_t* image_in,int in_w,int in_h,
