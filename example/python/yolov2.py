...
if __name__ == "__main__":
    ...
    if args.dataset=='VOC':
        ...
        if 'tiny' in args.model:
            predictions = yolo.yolov2_tiny_voc(outputs, model.output_scale_factor)
        else:
            predictions = yolo.yolov2_voc(outputs, model.output_scale_factor)
    else:
        classes = dataset.coco_classes
        colors = dataset.coco_colors
        predictions = yolo.yolov2_tiny_coco(outputs, model.output_scale_factor)

    output_img = cv2.resize(img, (1024, 1024), interpolation=cv2.INTER_NEAREST)
    output_scale = 1024. / 416
