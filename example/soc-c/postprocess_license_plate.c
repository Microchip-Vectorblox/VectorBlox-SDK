...
int post_process_lpd_int8(object_t objects[],int max_objects, int8_t *detectOutputs[9], int image_width, int image_height,
    ...
    int8_t *objMaps[3], *keyMaps[3], *boxMaps[3], *shapeOutput[3];
    ...
    for(int mapNum = 0; mapNum < 3; mapNum++) {
    	int8_t* objMap = objMaps[mapNum];
        int pixels = mapPixels[mapNum];
        for(int n = 0; n < pixels; n++) {
            scores[s] = int8_to_fix16_single((int8_t)objMap[n],scale_outs[2*mapNum],zero_points[2*mapNum]);
            s++;
        }
    }
    ...
    for(int n = 0; n < orderLength; n++) {
        ...
        fix16_t raw[6];
        int8_t* current_output_layer = shapeOutput[mapNum];
        for(int i = 0; i < 6; i++) {
        	raw[i] = int8_to_fix16_single(current_output_layer[i*mapPixels[mapNum]+ y*mapSizes[mapNum][1]+x],scale_outs[2*mapNum+1],zero_points[2*mapNum+1]);//double check mapNum 1/ 3/ 5
        }
        ...
        if (detectNumOutputs == 6) {  //6 output layers
            ...
            int8_t* locPtr = &boxMaps[mapNum][anchNum*4*pixels+ind];
            for(int nLoc = 0; nLoc < 4; nLoc++) {
            	location[nLoc] = int8_to_fix16_single(*locPtr,scale_outs[2*mapNum+1],zero_points[2*mapNum+1]);// * stride;
                locPtr += pixels;
...
fix16_t post_process_lpr_int8(int8_t *output, model_t *model, char *label) {
	fix16_t temp[666];
	int32_t zero_point = model_get_output_zeropoint(model,0);
	fix16_t scale = model_get_output_scale_fix16_value(model,0);
    int output_length = model_get_output_length(model, 0);
    int8_to_fix16(temp, (int8_t*)output, output_length, scale, zero_point);
	fix16_t conf =  PlateDecodeCStyle(temp, output_length, label);
	return conf;
}
